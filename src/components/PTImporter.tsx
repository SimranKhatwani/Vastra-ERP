...
import { Product, Supplier, PurchaseOrder, PurchaseOrderItem } from '../types/erp';

interface PTImporterProps {
  products: Product[];
...
interface ParsedRow {
  ...
  // mapping resolution
  resolution: 'create' | 'map' | 'none';
  mappedProductId?: string;
  generatedLotId?: string;
}

const FIELDS_TO_MAP = [
  { key: 'vendorName', label: 'Vendor Name', required: true, synonyms: ['vendor', 'supplier', 'wholesaler', 'seller', 'party', 'supplier name', 'vendor name'] },
  { key: 'invoiceNo', label: 'Invoice Number', required: true, synonyms: ['invoice', 'bill', 'inv', 'voucher', 'invoice no', 'bill no', 'invoice number', 'bill number'] },
  { key: 'invoiceDate', label: 'Invoice Date', required: true, synonyms: ['date', 'invoice date', 'bill date', 'dated', 'inv date'] },
  { key: 'fabricCode', label: 'Fabric Code', required: true, synonyms: ['code', 'fabric code', 'sku', 'item code', 'design', 'fabric no', 'quality no', 'quality code'] },
  { key: 'fabricName', label: 'Fabric Name', required: true, synonyms: ['name', 'fabric name', 'item name', 'description', 'fabric', 'item', 'quality name'] },
  { key: 'gsm', label: 'GSM', required: false, synonyms: ['gsm', 'density', 'weight', 'g/sm'] },
  { key: 'width', label: 'Width', required: false, synonyms: ['width', 'dia', 'size', 'width (inches)', 'width (dia)', 'inches'] },
  { key: 'color', label: 'Color', required: false, synonyms: ['color', 'colour', 'shade', 'hue', 'shade no', 'color code'] },
  { key: 'quantity', label: 'Quantity', required: true, synonyms: ['quantity', 'qty', 'meters', 'roll', 'rolls', 'mtrs', 'volume'] },
  { key: 'unit', label: 'Unit', required: false, synonyms: ['unit', 'uom', 'mtr', 'meters', 'pcs'] },
  { key: 'rate', label: 'Rate', required: true, synonyms: ['rate', 'price', 'cost', 'unit price', 'rate per mtr'] },
  { key: 'amount', label: 'Amount', required: false, synonyms: ['amount', 'value', 'subtotal', 'total price', 'basic amount'] },
  { key: 'tax', label: 'Tax (GST)', required: false, synonyms: ['tax', 'gst', 'cgst', 'sgst', 'igst', 'tax amount', 'vat', 'gst amount'] },
  { key: 'totalAmount', label: 'Total Amount', required: false, synonyms: ['total amount', 'grand total', 'invoice total', 'total', 'bill amount', 'net amount'] },
];

export const PTImporter: React.FC<PTImporterProps> = ({
  products,
  setProducts,
  suppliers,
  setSuppliers,
  ...
  const [editingData, setEditingData] = useState<ParsedRow | null>(null);

  // Success summary details
  const [createdVoucher, setCreatedVoucher] = useState<PurchaseOrder | null>(null);
  ...
  const importStats = useMemo(() => {
    ...
    let duplicates = parsedRows.filter(r => r.errors.some(e => e.includes('Duplicate Invoice'))).length;
  ...
  const processFile = (file: File) => {
    if (!file.name.endsWith('.xlsx') && !file.name.endsWith('.xls')) {
      onAddNotification('Invalid File', 'Please upload a valid Excel file (.xlsx or .xls)', 'danger');
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (rows.length === 0) {
          ...
          onAddNotification('Empty File', 'The uploaded Excel file contains no data.', 'danger');
          return;
        }

        const headers = (rows[0] || []).map(h => String(h).trim());
        const dataRows = rows.slice(1).filter(r => r.length > 0 && r.some(cell => cell !== null && cell !== undefined && cell !== ''));
        ...
        FIELDS_TO_MAP.forEach(field => {
          ...
          let matchIdx = headers.findIndex(h => h.toLowerCase() === field.key.toLowerCase() || h.toLowerCase() === field.label.toLowerCase());
          
          // Look for synonym matches
          if (matchIdx === -1) {
            matchIdx = headers.findIndex(h => 
              field.synonyms.some(syn => h.toLowerCase() === syn.toLowerCase() || h.toLowerCase().includes(syn.toLowerCase()))
            );
          }
        ...
        setTimeout(() => {
          setIsUploading(false);
          setStep('mapping');
          onAddNotification('Success', 'Excel parsed. Review column mappings.', 'success');
        }, 500);

      } catch (err) {
        clearInterval(interval);
        setIsUploading(false);
        onAddNotification('Parsing Failed', 'Error parsing Excel spreadsheet content.', 'danger');
        console.error(err);
      }
    };
  ...
  const handleConfirmMapping = () => {
    ...
    const parsed: ParsedRow[] = rawRows.map((rawRow, idx) => {
      ...
      const invoiceDate = getVal('invoiceDate') || new Date().toISOString().split('T')[0];
  ...
  const validateRows = (rowsToValidate: ParsedRow[]) => {
    const validated = rowsToValidate.map(row => {
      ...
      const duplicateInDB = purchaseOrders.some(po => po.invoiceNo?.toLowerCase() === row.invoiceNo.toLowerCase());
      if (duplicateInDB) {
      ...
      const duplicateInSheet = rowsToValidate.filter(r => r.tempId !== row.tempId && r.invoiceNo.toLowerCase() === row.invoiceNo.toLowerCase());
      if (duplicateInSheet.length > 0 && duplicateInSheet[0].vendorName.toLowerCase() !== row.vendorName.toLowerCase()) {
        errors.push(`Multiple vendors specified for same Invoice No (${row.invoiceNo})`);
      }

      // Check if fabric code already exists in Fabric Master (products list)
      const fabricInMaster = products.find(p => p.sku.toLowerCase() === row.fabricCode.toLowerCase() || p.id.toLowerCase() === row.fabricCode.toLowerCase());
      
      let resolution = row.resolution;
      let mappedProductId = row.mappedProductId;

      if (!fabricInMaster) {
        warnings.push(`Fabric Code "${row.fabricCode}" not found in Fabric Master`);
        if (resolution === 'none') {
      ...
      const supplierInDB = suppliers.find(s => s.name.toLowerCase() === row.vendorName.toLowerCase());
      if (!supplierInDB) {
        warnings.push(`Supplier "${row.vendorName}" is not registered (will be registered automatically)`);
      }
  ...
  const handleSaveRowEdit = () => {
    ...
    const updated = parsedRows.map(r => r.tempId === editingRowId ? { 
      ...editingData, 
      amount, 
      tax, 
      totalAmount 
    ...
    setEditingData(null);
    onAddNotification('Row Updated', 'Spreadsheet row details edited and re-validated.', 'success');
  };

  // Bulk Resolve unmatched fabrics
  const handleSetBulkResolution = (resolution: 'create' | 'map', mappedProductId?: string) => {
    const updated = parsedRows.map(row => {
      const fabricInMaster = products.find(p => p.sku.toLowerCase() === row.fabricCode.toLowerCase());
      if (!fabricInMaster) {
        return {
          ...row,
          resolution,
  ...
  const handleRowResolutionChange = (tempId: string, resolution: 'create' | 'map', mappedId?: string) => {
  ...
  const handleDeleteRow = (tempId: string) => {
    ...
    validateRows(updated);
    onAddNotification('Row Excluded', 'Excluded row from imported batch list.', 'info');
  };
  ...
  const handleImportPTFileSubmit = () => {
    ...
    if (importStats.error > 0) {
      onAddNotification('Import Blocked', 'Please resolve or delete rows with validation errors first.', 'danger');
      return;
    }

    if (parsedRows.length === 0) {
      onAddNotification('Empty List', 'No valid rows available to import.', 'warning');
      return;
    }

    // 2. Automatically Register New Suppliers if any doesn't exist
    let currentSuppliers = [...suppliers];
    const uniqueVendorsInImport: string[] = Array.from(new Set(parsedRows.map(r => r.vendorName))) as string[];
    
    uniqueVendorsInImport.forEach(vendor => {
      const exists = currentSuppliers.some(s => s.name.toLowerCase() === vendor.toLowerCase());
      if (!exists) {
    ...
    parsedRows.forEach(row => {
      if (row.resolution === 'create') {
        const alreadyExists = currentProducts.some(p => p.sku.toLowerCase() === row.fabricCode.toLowerCase());
        if (!alreadyExists) {
          const newFabricProd: Product = {
    ...
    const uniqueInvoiceNumbers: string[] = Array.from(new Set(parsedRows.map(r => r.invoiceNo))) as string[];
    let generatedPOs: PurchaseOrder[] = [];
    ...
    uniqueInvoiceNumbers.forEach(invoiceNum => {
      ...
      const matchedSupObj = currentSuppliers.find(s => s.name.toLowerCase() === representativeRow.vendorName.toLowerCase()) || currentSuppliers[0];

      // Build Items with unique Lot ID generation
      const items: PurchaseOrderItem[] = invoiceRows.map((r, itemIdx) => {
        // Find mapped productId in refreshed products list
        const prodObj = currentProducts.find(p => p.sku.toLowerCase() === r.fabricCode.toLowerCase());
...
<body>
  <div class="invoice-card">
    ...
    onAddNotification('Voucher Downloader', `Purchase Voucher HTML ${createdVoucher.poNo} downloaded.`, 'success');
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" id="pt-import-system-root">
      ...
      <div className="bg-slate-900 text-white p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider font-mono">Procurement Tools</span>
            <h2 className="text-base font-bold">PT (Purchase Template) File Import System</h2>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {/* STEP WIZARD PILLS */}
          <div className="hidden md:flex items-center gap-2 text-[10px] font-bold uppercase font-mono bg-slate-800/80 px-4 py-1.5 rounded-full border border-slate-700">
            <span className={step === 'upload' ? 'text-indigo-400' : 'text-slate-400'}>1. Upload</span>
            <ChevronRight className="w-2.5 h-2.5 text-slate-500" />
            <span className={step === 'mapping' ? 'text-indigo-400' : 'text-slate-400'}>2. Map Columns</span>
            <ChevronRight className="w-2.5 h-2.5 text-slate-500" />
            <span className={step === 'preview' ? 'text-indigo-400' : 'text-slate-400'}>3. Verify & Edit</span>
            <ChevronRight className="w-2.5 h-2.5 text-slate-500" />
            <span className={step === 'success' ? 'text-emerald-400 font-extrabold' : 'text-slate-400'}>4. Generated</span>
          </div>
        </div>
      </div>
        ...
        <div className="p-8 space-y-6">
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center space-y-2">
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Upload Supplier Purchase Template</h3>
              <p className="text-xs text-slate-400">
                Directly parse bulk sheets from Aura, Vardhman, Arvind, or other suppliers. The system auto-resolves material masters.
              </p>
            </div>
            ...
            >
              ...
              <div className="space-y-1">
                <p className="text-xs font-semibold text-slate-600">Drag & drop vendor PT spreadsheet here, or <span className="text-indigo-600 hover:underline">browse files</span></p>
                <p className="text-[10px] text-slate-400 font-mono">Supports .XLSX and .XLS file types</p>
              </div>
            </div>
            ...
            {isUploading && (
              <div className="space-y-2 animate-fade-in">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-medium text-slate-500 flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5 animate-spin text-indigo-500" /> Reading Worksheet Matrix...</span>
            ...
            <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 text-xs space-y-3.5">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed">
                The import wizard automatically looks for and processes the following columns. It supports synonyms and performs fuzzy matching on columns:
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 font-mono text-[10px]">
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Vendor Name (Vendor)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Invoice Number (Bill)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Fabric Code (SKU)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Fabric Name (Desc)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> GSM (Density)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Width (Dia)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Color / Shade</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Quantity (Meters)</div>
                <div className="p-2 bg-white rounded-lg border border-slate-100"><span className="text-indigo-600 font-bold">•</span> Rate (Price)</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {step === 'mapping' && (
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide">Fuzzy Column Alignment Map</h3>
              <p className="text-xs text-slate-400">Verify how Excel columns map to Garment ERP fields. Select columns manually if needed.</p>
            </div>
            <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-xs font-mono font-bold">{fileName}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-medium">Mapped to Excel header</p>
                    </div>

                    <div className="flex items-center gap-2">
                      <select 
                        value={mappedColIdx === undefined ? '' : String(mappedColIdx)}
                        onChange={(e) => {
                      ...
                      >
                        ...
                        {excelHeaders.map((header, idx) => (
                          <option key={idx} value={String(idx)}>Col {idx + 1}: {header}</option>
                        ))}
                      </select>
                      {isMapped ? (
                        <span className="bg-emerald-100 text-emerald-800 p-1.5 rounded-full" title="Mapped Successfully"><Check className="w-3.5 h-3.5" /></span>
                      ) : (
                        field.required && (
                          <span className="bg-rose-100 text-rose-800 p-1.5 rounded-full" title="Required Field Unmapped"><AlertTriangle className="w-3.5 h-3.5 text-rose-600" /></span>
                        )
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-5 space-y-4 text-xs">
              <h4 className="font-bold text-slate-700 uppercase tracking-wide flex items-center gap-1.5"><Eye className="w-4 h-4 text-indigo-500" /> Initial Raw Excel Headings Mapping</h4>
              <p className="text-slate-400 leading-relaxed text-[11px]">
                Below is a sample of headings found on your spreadsheet row 1. Double check mapping is correct before generating editable voucher lines.
              </p>
              
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {excelHeaders.map((header, idx) => {
                  // Find if mapped
                  const mappedField = FIELDS_TO_MAP.find(f => columnMapping[f.key] === idx);
                  return (
                    <div key={idx} className="p-2.5 bg-white rounded-xl border border-slate-100 flex items-center justify-between font-mono text-[11px]">
                      <div>
                        <span className="text-slate-400 mr-2">Col {idx + 1}:</span>
                        <span className="font-bold text-slate-700">{header}</span>
                      </div>
                      {mappedField ? (
                        <span className="bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded text-[9px] font-bold uppercase">{mappedField.label}</span>
                      ) : (
                        <span className="bg-slate-100 text-slate-400 px-2 py-0.5 rounded text-[9px] font-bold uppercase">Skipped</span>
                      )}
                    </div>
                  );
                })}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl flex items-center justify-between text-xs shadow-xs">
              <div>
                <span className="text-slate-400 font-bold uppercase tracking-wide block">Import Lines</span>
                <span className="text-xl font-black text-slate-800">{importStats.total} rows</span>
              </div>
              <div className="bg-indigo-100 text-indigo-600 p-2.5 rounded-xl"><FileSpreadsheet className="w-5 h-5" /></div>
            </div>
            ...
            <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-xl flex items-center justify-between text-xs shadow-xs">
              <div>
                <span className="text-emerald-500 font-bold uppercase tracking-wide block">Ready to Import</span>
                <span className="text-xl font-black text-emerald-700">{importStats.success} rows</span>
              </div>
              <div className="bg-emerald-100 text-emerald-600 p-2.5 rounded-xl"><CheckCircle2 className="w-5 h-5" /></div>
            </div>

            <div className="bg-amber-50/40 border border-amber-100 p-4 rounded-xl flex items-center justify-between text-xs shadow-xs">
              <div>
                <span className="text-amber-500 font-bold uppercase tracking-wide block">Fabric Warnings</span>
                <span className="text-xl font-black text-amber-700">{importStats.warning} rows</span>
              </div>
              <div className="bg-amber-100 text-amber-600 p-2.5 rounded-xl"><AlertTriangle className="w-5 h-5" /></div>
            </div>

            <div className="bg-rose-50/40 border border-rose-100 p-4 rounded-xl flex items-center justify-between text-xs shadow-xs">
              <div>
                <span className="text-rose-500 font-bold uppercase tracking-wide block">Validation Errors</span>
                <span className="text-xl font-black text-rose-700">{importStats.error} rows</span>
              </div>
              <div className="bg-rose-100 text-rose-600 p-2.5 rounded-xl"><XCircle className="w-5 h-5" /></div>
            </div>
          </div>
          ...
          {importStats.warning > 0 && (
            <div className="bg-amber-50/60 border border-amber-200/50 rounded-xl p-4 text-xs space-y-3">
              <div className="flex items-center gap-2 text-amber-800 font-bold">
                ...
                <span>Fabric Master Sync Warning: Fabric codes do not exist in system database</span>
              </div>
              <p className="text-[11px] text-slate-500">
                You have imported fabric materials that do not exist in the Fabric Master. Choose how you want to handle these unmatched fabric codes:
              </p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Voucher spreadsheet verification lines</span>
              <span className="text-[10px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded font-bold">Spreadsheet Style</span>
            </div>
              ...
              <table className="w-full text-left text-[11px] border-collapse">
                <thead>
                  <tr className="bg-slate-100/80 text-slate-500 font-bold border-b border-slate-200 uppercase tracking-wide select-none">
                    <th className="p-3 text-center border-r border-slate-200 w-10">Row</th>
                    <th className="p-3 border-r border-slate-200 min-w-[70px]">Status</th>
                    <th className="p-3 border-r border-slate-200 min-w-[120px]">Vendor (Party)</th>
                    <th className="p-3 border-r border-slate-200 font-mono min-w-[90px]">Invoice No</th>
                    <th className="p-3 border-r border-slate-200 font-mono min-w-[110px]">Fabric SKU (Code)</th>
                    <th className="p-3 border-r border-slate-200 min-w-[150px]">Fabric Name</th>
                    <th className="p-3 border-r border-slate-200 text-center w-14">GSM</th>
                    <th className="p-3 border-r border-slate-200 text-center w-14">Width</th>
                    <th className="p-3 border-r border-slate-200 min-w-[80px]">Color</th>
                    <th className="p-3 border-r border-slate-200 text-right min-w-[80px]">Quantity</th>
                    <th className="p-3 border-r border-slate-200 text-right min-w-[80px]">Rate (₹)</th>
                    <th className="p-3 border-r border-slate-200 text-right min-w-[90px]">Total (Tax Incl)</th>
                    <th className="p-3 min-w-[140px] text-center">Unmatched Resolution</th>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {parsedRows.map((row, idx) => {
                    ...
                    return (
                      ...
                      >
                        {/* ROW INDEX */}
                        <td className="p-3 text-center border-r border-slate-100 font-mono text-slate-400">{idx + 1}</td>
                        ...
                        <td className="p-3 border-r border-slate-100">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, vendorName: e.target.value } : null)}
                        ...
                        <td className="p-3 border-r border-slate-100 font-mono">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, invoiceNo: e.target.value } : null)}
                        ...
                        <td className="p-3 border-r border-slate-100 font-mono font-bold text-slate-700">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, fabricCode: e.target.value } : null)}
                              className="w-full bg-slate-50 border border-slate-200 rounded px-1.5 py-1 font-mono"
                            />
                          ) : (
                            row.fabricCode
                        ...
                        <td className="p-3 border-r border-slate-100">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, fabricName: e.target.value } : null)}
                              className="w-full bg-slate-50 border border-slate-200 rounded px-1.5 py-1"
                            />
                          ) : (
                            row.fabricName
                        ...
                        <td className="p-3 border-r border-slate-100 text-center font-mono">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, gsm: Number(e.target.value) } : null)}
                        ...
                        <td className="p-3 border-r border-slate-100 text-center font-mono">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, width: Number(e.target.value) } : null)}
                              className="w-full bg-slate-50 border border-slate-200 rounded px-1 py-1 font-mono text-center"
                            />
                          ) : (
                            row.width ? `${row.width}"` : <span className="text-slate-300">-</span>
                          )}
                        </td>
                        ...
                        <td className="p-3 border-r border-slate-100 text-right font-mono text-slate-700">
                          {isEditing ? (
                            <input 
                              ...
                              onChange={(e) => setEditingData(prev => prev ? { ...prev, rate: Number(e.target.value) } : null)}
                              className="w-full bg-slate-50 border border-slate-200 rounded px-1.5 py-1 text-right font-mono"
                            />
                          ) : (
                        ...
                        <td className="p-3 border-r border-slate-100 text-center">
                          {products.some(p => p.sku.toLowerCase() === row.fabricCode.toLowerCase()) ? (
                            <span className="text-emerald-600 font-bold flex items-center justify-center gap-1"><Check className="w-3.5 h-3.5" /> Matched In DB</span>
                          ) : (
                            <div className="flex items-center gap-1.5 justify-center">
                              <select 
                                value={row.resolution}
                                onChange={(e) => handleRowResolutionChange(row.tempId, e.target.value as any)}
                                className="bg-slate-50 border border-slate-200 rounded px-1.5 py-1 text-[10px] font-semibold text-slate-700"
                              >
                                <option value="create">Auto-Create Fabric</option>
                              ...
                              {row.resolution === 'map' && (
                                <select
                                  ...
                                  onChange={(e) => handleRowResolutionChange(row.tempId, 'map', e.target.value)}
            ...
            {parsedRows.some(r => r.errors.length > 0) && (
              <div className="bg-rose-50 border-t border-rose-200 p-4 text-xs space-y-2">
                <div className="flex items-center gap-2 text-rose-800 font-bold">
                  ...
                  <span>Row Validation Failures: Resolve conflicts to continue import</span>
                </div>
                <div className="max-h-24 overflow-y-auto space-y-1.5 font-mono text-[10px] text-rose-700">
                  {parsedRows.filter(r => r.errors.length > 0).map((r, i) => (
                    <div key={i} className="flex gap-2">
                      <span className="font-extrabold bg-rose-200 text-rose-800 px-1 rounded">Row {i+1}</span>
      ...
      {step === 'success' && createdVoucher && (
        <div className="p-8 space-y-8 animate-fade-in text-xs max-w-4xl mx-auto">
          ...
          <div className="text-center space-y-3">
            <div className="w-16 h-16 bg-emerald-500 text-white rounded-full flex items-center justify-center mx-auto text-3xl shadow-lg border-4 border-emerald-100 animate-bounce">✓</div>
            <div className="space-y-1">
              <h3 className="text-base font-black text-slate-800 uppercase tracking-wide">Purchase Template Imported Successfully!</h3>
              <p className="text-slate-400">Unique stock lots have been created, fabric inventory loaded, and supply ledgers synced.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl space-y-1.5 shadow-xs">
              <span className="text-slate-400 font-bold uppercase tracking-wide block">Purchase Voucher Generated</span>
              <span className="text-sm font-bold text-slate-800 font-mono">{createdVoucher.poNo}</span>
            </div>

            <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl space-y-1.5 shadow-xs">
              <span className="text-slate-400 font-bold uppercase tracking-wide block">Vendor Invoice Reference</span>
              <span className="text-sm font-bold text-slate-800 font-mono">{createdVoucher.invoiceNo || 'N/A'}</span>
            </div>

            <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl space-y-1.5 shadow-xs">
              <span className="text-slate-400 font-bold uppercase tracking-wide block">Grand Total Saved to Ledger</span>
              <span className="text-sm font-bold text-indigo-600 font-mono">₹{createdVoucher.grandTotal.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="bg-indigo-50/40 border border-indigo-100 rounded-2xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 font-mono text-[10px] text-indigo-700">
              {createdVoucher.items.map((item, idx) => (
                <div key={idx} className="p-2.5 bg-white border border-indigo-100 rounded-xl flex flex-col justify-between shadow-2xs">
                  <span className="font-extrabold text-slate-800">{item.name.split(' (')[0]}</span>
                  <div className="flex justify-between items-center mt-1.5 pt-1 border-t border-slate-100">
                    ...
                    <span className="bg-indigo-100 text-indigo-800 px-1.5 py-0.5 rounded font-bold">{item.lotId || 'N/A'}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 p-6 shadow-sm space-y-4">
            ...
            <div className="bg-white border border-slate-200 p-8 rounded-xl max-w-3xl mx-auto space-y-6 shadow-xs font-sans text-slate-700 leading-normal">
              ...
              <div className="flex justify-between items-start border-b-2 border-slate-100 pb-4">
                <div>
                  <h4 className="text-slate-900 font-extrabold text-base tracking-tight uppercase">Garment ERP Systems</h4>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono">Wholesale Textile Procurement Loop</p>
                </div>
                <div className="text-right">
                  <h4 className="text-indigo-600 font-bold tracking-tight uppercase">Purchase Voucher</h4>
                  <p className="text-[10px] text-slate-500 mt-1 font-mono">VOUCHER: <strong className="text-slate-800 font-bold">{createdVoucher.poNo}</strong></p>
                  <p className="text-[10px] text-slate-400 font-mono">DATE: {createdVoucher.date}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <h5 className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Wholesale Supplier Account</h5>
                  <p className="font-extrabold text-slate-800">{createdVoucher.supplierName}</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Supply Chain Ledger Account: Updated & Settled</p>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <h5 className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">PT File Metadata Summary</h5>
                  <p className="font-mono text-slate-800">Invoice Ref No: <strong>{createdVoucher.invoiceNo || 'N/A'}</strong></p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Date Raised: {createdVoucher.invoiceDate || 'N/A'}</p>
                </div>
              </div>
                  ...
                  {createdVoucher.items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-2.5">
                        ...
                        <span className="text-[9px] text-slate-400">SKU: {item.fabricCode} | Tax rate: 12% GST</span>
                      </td>
                      <td className="p-2.5 text-center"><span className="bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded font-mono font-bold">{item.lotId}</span></td>
                      <td className="p-2.5 text-right font-mono">{item.quantity.toLocaleString()} {item.unit || 'Mtr'}</td>
                      <td className="p-2.5 text-right font-mono">₹{item.purchasePrice.toLocaleString()}</td>
                      <td className="p-2.5 text-right font-mono font-bold text-slate-800">₹{item.totalPrice.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
