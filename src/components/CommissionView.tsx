...
import { Employee, Customer, Invoice } from '../types/erp';

// High fidelity types for Commission Module
export interface MarketplaceOrder {
  id: string;
  marketplace: 'Amazon' | 'Meesho' | 'Flipkart' | 'Myntra' | 'Ajio' | 'Shopify Orders' | 'Custom';
  orderId: string;
  invoiceNo: string;
  customerName: string;
  productName: string;
  ...
  orderStatus: 'Delivered' | 'Returned' | 'Shipped' | 'Pending';
  settlementStatus: 'Settled' | 'Pending' | 'Disputed';
...
export interface InfluencerCampaign {
  ...
  commissionValue: number; // % or flat ₹
  duration: string;
...
export interface SalespersonIncentive {
  employeeId: string;
  employeeName: string;
  department: string;
  role: string;
...
export interface SettlementHistoryItem {
  id: string;
  date: string;
  type: 'Marketplace' | 'Influencer' | 'Salesperson';
  recipient: string;
  referenceNo: string;
  amount: number;
  paymentMethod: 'Bank Transfer' | 'UPI' | 'Store Wallet' | 'Cash';
...
export const CommissionView: React.FC<CommissionViewProps> = ({
  ...
  const [soldProductsSearchQuery, setSoldProductsSearchQuery] = useState<string>('');
  ...
  const salespeopleNames = useMemo(() => {
    ...
    employees.filter(e => e.role === 'Salesperson').forEach(e => namesSet.add(e.name));
  ...
  const salespersonSalesHistory = useMemo(() => {
    const history: { [salespersonName: string]: Array<{
      id: string;
      invoiceNo: string;
      date: string;
      customerName: string;
    ...
    invoices.forEach(inv => {
      ...
      if (spName) {
        ...
        const match = salespeopleNames.find(n => n.toLowerCase() === spName.toLowerCase()) || spName;
        if (!history[match]) {
          history[match] = [];
        }
  ...
  const [newCatType, setNewCatType] = useState<'Percentage' | 'Fixed'>('Percentage');
  const [newCatValue, setNewCatValue] = useState(10);

  // Dynamic category partner & record states
  const [categoryPartners, setCategoryPartners] = useState<any[]>([
    { id: 'cp-1', categoryId: 'b2b-agents', name: 'Elite Garments Agency', referenceNo: 'B2B-ELITE', phone: '9822334455', email: 'elite@agency.com', status: 'Active', revenue: 150000, commissionEarned: 15000 },
    { id: 'cp-2', categoryId: 'b2b-agents', name: 'Sovereign Exports', referenceNo: 'B2B-SOV', phone: '9988776655', email: 'sov@exports.com', status: 'Active', revenue: 320000, commissionEarned: 32000 }
  ]);
  const [categoryRecords, setCategoryRecords] = useState<any[]>([
    { id: 'cr-1', categoryId: 'b2b-agents', partnerId: 'cp-1', partnerName: 'Elite Garments Agency', referenceNo: 'REF-8891', saleAmount: 85000, commissionAmount: 8500, status: 'Settled', date: '2026-06-24' },
    { id: 'cr-2', categoryId: 'b2b-agents', partnerId: 'cp-2', partnerName: 'Sovereign Exports', referenceNo: 'REF-9402', saleAmount: 120000, commissionAmount: 12000, status: 'Pending', date: '2026-06-27' }
  ]);
  ...
  const [mpMarketplace, setMpMarketplace] = useState<'Amazon' | 'Meesho' | 'Flipkart' | 'Myntra' | 'Ajio' | 'Shopify Orders' | 'Custom'>('Amazon');
  ...
  const [infPlatform, setInfPlatform] = useState<'Instagram' | 'YouTube' | 'TikTok' | 'Facebook'>('Instagram');
  ...
  const [infCommissionType, setInfCommissionType] = useState<'Percentage' | 'Fixed'>('Percentage');
  ...
  const [influencers, setInfluencers] = useState<InfluencerCampaign[]>(() => [
    {
      id: 'inf-1',
      name: 'Diya Krishna',
      platform: 'Instagram',
  ...
  const [salespersonIncentives, setSalespersonIncentives] = useState<SalespersonIncentive[]>(() => [
    {
      employeeId: 'e-3',
      ...
      rating: 4.7,
      commissionRules: ['2% Floor Commission', '₹1,500 Bonus above ₹1.5L Target', 'Product Category Commission (Casual Shirts 3%)'],
    ...
    },
    {
      ...
      rating: 4.9,
      commissionRules: ['5% Custom Tailoring Commission', 'Flat ₹500 Pattern-weaving Bonus'],
    ...
    },
    {
      ...
      rating: 4.8,
      commissionRules: ['1.5% Wholesale Bulk Volume Commission', '₹3,000 Target Achievement Reward'],
  ...
  const [auditLogs, setAuditLogs] = useState<AuditLogItem[]>(() => [
    ...
    },
    {
      ...
      details: 'Created custom rule: "Product Category Commission (Casual Shirts 3%)" for Floor sales staff.'
    }
  ]);

  const [commissionRulesList, setCommissionRulesList] = useState<string[]>(() => [
  ...
  // Master stats counts
  const totalMarketplaceSales = useMemo(() => marketplaceOrders.reduce((sum, item) => sum + item.sellingPrice, 0), [marketplaceOrders]);
  const totalMarketplaceCommission = useMemo(() => marketplaceOrders.reduce((sum, item) => sum + item.commissionAmount, 0), [marketplaceOrders]);
  const totalMarketplaceNetSettlement = useMemo(() => marketplaceOrders.reduce((sum, item) => sum + item.netSettlement, 0), [marketplaceOrders]);
  const pendingMarketplaceSettlement = useMemo(() => marketplaceOrders.filter(m => m.settlementStatus === 'Pending').reduce((sum, item) => sum + item.netSettlement, 0), [marketplaceOrders]);

  const totalInfluencerSales = useMemo(() => influencers.reduce((sum, item) => sum + item.revenueGenerated, 0), [influencers]);
  const totalInfluencerCommission = useMemo(() => influencers.reduce((sum, item) => sum + item.commissionEarned, 0), [influencers]);
  const pendingInfluencerCommission = useMemo(() => influencers.reduce((sum, item) => sum + item.commissionPending, 0), [influencers]);

  const totalSalespersonCommissions = useMemo(() => salespersonIncentives.reduce((sum, item) => sum + item.monthlyCommission, 0), [salespersonIncentives]);
  const pendingSalespersonCommissions = useMemo(() => salespersonIncentives.reduce((sum, item) => sum + item.commissionPending, 0), [salespersonIncentives]);
  ...
  const sortedMarketplaceOrders = useMemo(() => {
    ...
    // Filter by Status
    if (statusFilter !== 'All') {
      res = res.filter(item => item.settlementStatus === statusFilter || item.orderStatus === statusFilter);
    }
    ...
    return res;
  }, [marketplaceOrders, searchQuery, channelFilter, statusFilter, sortBy, sortOrder]);
  ...
  // Handle CRUD
  const handleAddMarketplaceOrder = (e: React.FormEvent) => {
    e.preventDefault();
    if (!mpOrderId || !mpCustomerName || !mpProductName || !mpSellingPrice) {
      onAddNotification('Form Validation', 'Please fill in all mandatory billing fields.', 'warning');
      return;
    }

    const calculatedCommission = Math.round((mpSellingPrice * mpCommissionPercent) / 100 * 10) / 10;
    const gstAmt = Math.round(mpSellingPrice * (mpTax / 100) * 10) / 10;
    const settlement = mpSellingPrice - calculatedCommission - mpShipping - mpPackaging - gstAmt;
    ...
    onAddNotification('Marketplace Order Recorded', `Added order ${mpOrderId} on ${mpMarketplace} successfully.`, 'success');
  ...
  const handleAddInfluencer = (e: React.FormEvent) => {
    ...
    if (!infName || !infHandle || !infReferralCode) {
      onAddNotification('Form Validation', 'Please specify Name, Social Handle, and Coupon Code.', 'warning');
      return;
    }

    const newInf: InfluencerCampaign = {
      id: `inf-${influencers.length + 101}`,
      name: infName,
      platform: infPlatform,
      handle: infHandle,
      phone: infPhone || '9876543211',
      email: infEmail || `${infName.toLowerCase().replace(/\s+/g, '')}@gmail.com`,
    ...
    onAddNotification('Campaign Activated', `Influencer ${infName} coupon ${newInf.referralCode} is now active.`, 'success');
  ...
  const handleApproveSettlement = (type: 'marketplace' | 'influencer', id: string) => {
    if (type === 'marketplace') {
      setMarketplaceOrders(prev => prev.map(o => {
        if (o.id === id) {
          onAddNotification('Settlement Cleared', `Order ${o.orderId} net settlement marked as PAID.`, 'success');
          
          // Record to Settlement history
          const hist: SettlementHistoryItem = {
          ...
          return { ...o, settlementStatus: 'Settled', settlementDate: '2026-06-28' };
        }
        return o;
      }));
    } else {
      setInfluencers(prev => prev.map(inf => {
        if (inf.id === id) {
          if (inf.commissionPending <= 0) {
            onAddNotification('Commissions Zero', 'No pending commission remains for approval.', 'warning');
            return inf;
          }
          const payoutAmount = inf.commissionPending;
          onAddNotification('Influencer Commission Paid', `Issued ₹${payoutAmount.toLocaleString()} to ${inf.name}.`, 'success');
  ...
  const handleApproveSalespersonPayout = (employeeId: string) => {
    setSalespersonIncentives(prev => prev.map(emp => {
      if (emp.employeeId === employeeId) {
        if (emp.commissionPending <= 0) {
          onAddNotification('No Dues', 'Commission is already fully disbursed.', 'info');
          return emp;
        }
        const disburseAmt = emp.commissionPending;
        onAddNotification('Commission Disbursed', `Disbursed ₹${disburseAmt.toLocaleString()} incentive to ${emp.employeeName}.`, 'success');
  ...
  const handleCreateCategorySubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCatName) {
      onAddNotification('Validation Error', 'Category name is required.', 'warning');
      return;
    }
    const catId = newCatName.toLowerCase().replace(/\s+/g, '-');
    if (customCategories.some(c => c.id === catId) || ['salesperson', 'rules', 'history', 'marketplace', 'influencer'].includes(catId)) {
      onAddNotification('Validation Error', 'Category name already exists or is reserved.', 'warning');
      return;
    }
    const newCat = {
      id: catId,
    ...
    setNewCatValue(10);
    onAddNotification('Category Created', `Dynamic category "${newCatName}" was created successfully.`, 'success');
  };

  const handleAddDynamicPartner = (e: React.FormEvent) => {
    e.preventDefault();
    if (!partnerName || !partnerCode) {
      onAddNotification('Validation Error', 'Partner Name and Referral Code/Reference are mandatory.', 'warning');
      return;
    }
    const newPartner = {
      id: `cp-${Date.now()}`,
      ...
      email: partnerEmail || `${partnerName.toLowerCase().replace(/\s+/g, '')}@partner.com`,
      status: 'Active',
      revenue: 0,
      commissionEarned: 0
    };
    ...
    setPartnerPhone('');
    setPartnerEmail('');
    onAddNotification('Partner Enrolled', `Enrolled partner "${partnerName}" under active category.`, 'success');
  };

  const handleAddDynamicRecord = (e: React.FormEvent) => {
    e.preventDefault();
    const partner = categoryPartners.find(p => p.id === recordPartnerId);
    if (!partner || recordSaleAmt <= 0) {
      onAddNotification('Validation Error', 'Please select a valid partner and specify sale amount.', 'warning');
      return;
    }
    ...
    const newRecord = {
      ...
      referenceNo: recordRefNo || `TXN-${Math.floor(Math.random() * 90000) + 10000}`,
    ...
    setCategoryPartners(prev => prev.map(p => p.id === partner.id ? {
      ...p,
    ...
    setRecordRefNo('');
    setRecordSaleAmt(0);
    onAddNotification('Record Created', `Created commission record of ₹${commAmt.toLocaleString()} for ${partner.name}.`, 'success');
  };

  const handleApproveDynamicSettlement = (recordId: string) => {
    setCategoryRecords(prev => prev.map(rec => {
      if (rec.id === recordId) {
        onAddNotification('Payout Disbursed', `Settlement of ₹${rec.commissionAmount.toLocaleString()} paid successfully to ${rec.partnerName}.`, 'success');
  ...
  const handleDeleteItem = (type: string, id: string) => {
    if (type === 'marketplace') {
      ...
      onAddNotification('Deleted Order', 'Marketplace order removed from ledger.', 'danger');
    } else if (type === 'influencer') {
      setInfluencers(prev => prev.filter(i => i.id !== id));
      onAddNotification('Campaign Terminated', 'Influencer campaign removed.', 'danger');
    }
    setShowDeleteConfirm(null);
  };

  const handleExportSimulated = (format: 'Excel' | 'PDF', moduleName: string) => {
    onAddNotification(
      'Export In Progress', 
    ...
    <div className="space-y-6 animate-fade-in pb-12" id="commission-mgmt-root">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Reconcile marketplace commissions, influencer referral campaigns, and salesperson performance bonuses.</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            onClick={() => handleExportSimulated('Excel', activeTab.toUpperCase())}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4.5">
          ...
          <div className="min-w-0">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold">Marketplace Net Sales</p>
            <h3 className="text-base font-extrabold text-slate-800 font-mono mt-0.5">₹{totalMarketplaceSales.toLocaleString('en-IN')}</h3>
            <span className="text-[9px] text-red-500 font-bold font-mono">Commission: ₹{totalMarketplaceCommission.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4.5">
          ...
          <div className="min-w-0">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold">Influencer Campaign Sales</p>
            <h3 className="text-base font-extrabold text-slate-800 font-mono mt-0.5">₹{totalInfluencerSales.toLocaleString('en-IN')}</h3>
            <span className="text-[9px] text-emerald-600 font-bold font-mono">Disbursed: ₹{(totalInfluencerCommission - pendingInfluencerCommission).toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4.5">
          ...
          <div className="min-w-0">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold">Salesperson Incentives</p>
            <h3 className="text-base font-extrabold text-slate-800 font-mono mt-0.5">₹{totalSalespersonCommissions.toLocaleString('en-IN')}</h3>
            <span className="text-[9px] text-amber-600 font-bold font-mono">Pending payout: ₹{pendingSalespersonCommissions.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4.5">
          ...
          <div className="min-w-0">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold">Total Pending Settlements</p>
            <h3 className="text-base font-extrabold text-indigo-600 font-mono mt-0.5">
              ₹{(pendingMarketplaceSettlement + pendingInfluencerCommission + pendingSalespersonCommissions).toLocaleString()}
            </h3>
            <span className="text-[9px] text-indigo-600 font-bold">Unreleased liabilities</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap border-b border-slate-100 pb-px items-center justify-between gap-3">
        <div className="flex flex-wrap gap-1 bg-slate-100/60 p-1 rounded-xl">
          <button 
            onClick={() => { setActiveTab('salesperson'); setSearchQuery(''); setStatusFilter('All'); }}
          ...
          {customCategories.map((cat, i) => (
            <button 
              key={cat.id}
              onClick={() => { setActiveTab(cat.id); setSearchQuery(''); setStatusFilter('All'); }}
          ...
          <button 
            onClick={() => { setActiveTab('rules'); setSearchQuery(''); setStatusFilter('All'); }}
          ...
          <button 
            onClick={() => { setActiveTab('history'); setSearchQuery(''); setStatusFilter('All'); }}
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-xs">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(searchQuery || statusFilter !== 'All' || channelFilter !== 'All') && (
              <button 
                onClick={() => { setSearchQuery(''); setStatusFilter('All'); setChannelFilter('All'); }}
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50/75 border-b border-slate-100 text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">
                  <th className="p-3.5 cursor-pointer select-none" onClick={() => requestSort('marketplace')}>
                    Marketplace {sortBy === 'marketplace' && (sortOrder === 'asc' ? '▲' : '▼')}
                  </th>
                  <th className="p-3.5">Order Info</th>
                  <th className="p-3.5">Customer & Product</th>
                  <th className="p-3.5 text-right cursor-pointer select-none" onClick={() => requestSort('sellingPrice')}>
                  ...
                  <th className="p-3.5 text-right font-bold text-indigo-600">Net Settlement</th>
                ...
                {sortedMarketplaceOrders.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="p-8 text-center text-slate-400">
                      ...
                      <p className="font-semibold">No marketplace order records found.</p>
                      <p className="text-[10px]">Try resetting filters or record a new channel sale.</p>
                    </td>
                  </tr>
                ) : (
                  sortedMarketplaceOrders.map(order => (
                    <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="p-3.5">
                        <div className="flex items-center gap-2">
                          <span className={`w-2.5 h-2.5 rounded-full ${
                            order.marketplace === 'Amazon' ? 'bg-amber-500' :
                            order.marketplace === 'Myntra' ? 'bg-rose-500' :
                            order.marketplace === 'Meesho' ? 'bg-purple-600' :
                            order.marketplace === 'Flipkart' ? 'bg-blue-500' :
                            'bg-emerald-500'
                          }`} />
                          <div>
                            <p className="font-extrabold text-slate-700">{order.marketplace}</p>
                      ...
                      <td className="p-3.5">
                        ...
                        <p className="text-[10px] text-slate-400 font-mono">Invoice: {order.invoiceNo}</p>
                      </td>
                      <td className="p-3.5 max-w-[200px]">
                        <p className="font-bold text-slate-700 truncate">{order.customerName}</p>
                        <p className="text-[10px] text-slate-400 truncate" title={order.productName}>{order.productName}</p>
                      </td>
                      <td className="p-3.5 text-right font-mono font-bold text-slate-700">
                        ₹{order.sellingPrice.toLocaleString()}
                      </td>
                      <td className="p-3.5 text-right">
                        <p className="font-semibold text-red-600 font-mono">₹{order.commissionAmount.toLocaleString()}</p>
                        <p className="text-[9px] text-slate-400">{order.commissionPercent}% charge</p>
                      </td>
                      <td className="p-3.5 text-right text-slate-500 font-mono">
                        <p>₹{order.shippingCharges}</p>
                        <p className="text-[9px]">Pkg: ₹{order.packagingCharges}</p>
                      </td>
                      ...
                      <td className="p-3.5 text-center">
                        <div className="flex flex-col items-center gap-1">
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                            ...
                            order.orderStatus === 'Returned' ? 'bg-red-50 text-red-600' :
                            'bg-amber-50 text-amber-600'
                          }`}>
                            {order.orderStatus}
                          </span>
                          <span className={`px-2 py-0.2 rounded text-[8px] font-extrabold uppercase ${
                            order.settlementStatus === 'Settled' ? 'bg-indigo-50 text-indigo-600' :
                            order.settlementStatus === 'Disputed' ? 'bg-red-100 text-red-700 border border-red-200' :
                            'bg-amber-100 text-amber-700 border border-amber-200'
                          }`}>
                            {order.settlementStatus === 'Settled' ? `Paid (${order.settlementDate})` : order.settlementStatus}
                          </span>
                        </div>
                      </td>
                      <td className="p-3.5 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          {order.settlementStatus !== 'Settled' && (
                            <button 
                              onClick={() => handleApproveSettlement('marketplace', order.id)}
                          ...
                          <button 
                            onClick={() => setShowDeleteConfirm({ type: 'marketplace', id: order.id })}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5 text-white relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="border border-slate-800 bg-slate-950/40 p-3.5 rounded-xl text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Total Promo Uses</p>
                <p className="text-xl font-bold mt-1 font-mono text-amber-400">
                  {influencers.reduce((sum, item) => sum + item.promoCodeUsage, 0)} orders
                </p>
              </div>
              <div className="border border-slate-800 bg-slate-950/40 p-3.5 rounded-xl text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Total Revenue Generated</p>
                <p className="text-xl font-bold mt-1 font-mono text-emerald-400">
                  ₹{totalInfluencerSales.toLocaleString()}
                </p>
              </div>
              <div className="border border-slate-800 bg-slate-950/40 p-3.5 rounded-xl text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Average Campaign Yield</p>
                <p className="text-xl font-bold mt-1 font-mono text-indigo-400">
                  ₹{Math.round(totalInfluencerSales / influencers.length).toLocaleString()}
                </p>
              </div>
              <div className="border border-slate-800 bg-slate-950/40 p-3.5 rounded-xl text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Avg Conversion Rate</p>
                <p className="text-xl font-bold mt-1 font-mono text-cyan-400">
                  {(influencers.reduce((sum, item) => sum + item.conversionRate, 0) / influencers.length).toFixed(1)}%
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredInfluencers.map(inf => (
              <div key={inf.id} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4 hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-extrabold text-slate-800 text-sm leading-tight">{inf.name}</h4>
                      <p className="text-indigo-600 font-semibold text-[11px] mt-0.5">@{inf.handle} ({inf.platform})</p>
                      <p className="text-[10px] text-slate-400">Followers: {(inf.followers/1000).toFixed(0)}K • Rating: ⭐ {inf.rating}</p>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[9px] text-slate-400 font-semibold mt-1">
                      {inf.commissionType === 'Percentage' ? `${inf.commissionValue}% share` : `₹${inf.commissionValue}/sale`}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2.5 bg-slate-50 p-3 rounded-xl text-center font-mono text-[11px]">
                  <div>
                    <span className="text-[8px] text-slate-400 uppercase tracking-wider block font-sans">Orders</span>
                    <span className="font-bold text-slate-800">{inf.ordersGenerated}</span>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-400 uppercase tracking-wider block font-sans">Sales Yield</span>
                    <span className="font-bold text-emerald-600">₹{inf.revenueGenerated.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-400 uppercase tracking-wider block font-sans">Total Earned</span>
                    <span className="font-bold text-red-600">₹{inf.commissionEarned.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center text-xs">
                  <div>
                    <p className="text-emerald-600 font-bold">Paid: ₹{inf.commissionPaid.toLocaleString()}</p>
                    <p className="text-amber-600 font-extrabold">Pending: ₹{inf.commissionPending.toLocaleString()}</p>
                  </div>

                  <div className="flex gap-1.5">
                    {inf.commissionPending > 0 && (
                      <button 
                        onClick={() => handleApproveSettlement('influencer', inf.id)}
                    ...
                    <button 
                      onClick={() => setShowDeleteConfirm({ type: 'influencer', id: inf.id })}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-xs">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Internal Employee Commission & Achievements</h3>
                <p className="text-[10px] text-slate-400">Configured target thresholds vs actual sales recorded at boutique counters.</p>
              </div>
              <span className="text-[9px] bg-indigo-50 text-indigo-700 font-bold px-2 py-0.5 rounded">Active staff: {salespersonIncentives.length}</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {salespersonIncentives.map(emp => {
                    const achievementPct = Math.round((emp.salesAchieved / emp.monthlyTarget) * 100);
                    return (
                      <tr key={emp.employeeId} className="hover:bg-slate-50/40">
                        <td className="p-3.5">
                          <p className="font-bold text-slate-800">{emp.employeeName}</p>
                          <p className="text-[9px] font-mono text-slate-400">ID: {emp.employeeId}</p>
                        </td>
                        <td className="p-3.5">
                          <p className="font-semibold text-slate-600">{emp.department}</p>
                          <p className="text-[9px] font-mono text-indigo-600 uppercase font-bold">{emp.role}</p>
                        </td>
                        <td className="p-3.5 text-right font-mono font-bold text-slate-700">
                        ...
                        <td className="p-3.5">
                          <div className="space-y-1 w-28">
                            <div className="flex justify-between text-[9px] font-bold font-mono">
                              <span className={achievementPct >= 100 ? 'text-emerald-600 animate-pulse' : 'text-amber-600'}>
                                {achievementPct}%
                              </span>
                        ...
                        <td className="p-3.5 max-w-[180px]">
                          <div className="flex flex-wrap gap-1">
                            {emp.commissionRules.map((rule, ri) => (
                              <span key={ri} className="bg-slate-50 text-[8px] text-slate-500 border border-slate-200/60 px-1 py-0.2 rounded truncate max-w-[170px]" title={rule}>
                                {rule}
                              </span>
                            ))}
                          </div>
                        ...
                        <td className="p-3.5 text-center">
                          {emp.commissionPending > 0 ? (
                            <button 
                              onClick={() => handleApproveSalespersonPayout(emp.employeeId)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
            ...
            <div className="lg:col-span-4 bg-white border border-slate-100 rounded-2xl p-5 shadow-xs space-y-4">
              <div className="border-b border-slate-100 pb-3">
                ...
                <p className="text-[10px] text-slate-400">Select any sales representative to fetch their real-time boutique checkout history.</p>
              </div>
              ...
              <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
                {salespeopleNames.length === 0 ? (
                  ...
                  salespeopleNames.map(name => {
                    ...
                    const qtySold = historyList.reduce((sum, item) => sum + item.quantity, 0);
                    const rev = historyList.reduce((sum, item) => sum + item.totalPrice, 0);
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs transition-colors ${
                            ...
                            {name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'SP'}
                          </div>
                          <div>
                            <p className="text-xs font-bold leading-tight truncate max-w-[140px]">{name}</p>
                            <p className="text-[9px] text-slate-400 font-semibold mt-0.5">{qtySold} items sold till date</p>
                          </div>
                        </div>
                        <div className="text-right font-mono">
                          <span className={`text-[10px] font-extrabold block ${isSelected ? 'text-indigo-700' : 'text-slate-800'}`}>
                            ₹{rev.toLocaleString()}
                          </span>
                          <span className="text-[8px] text-slate-400 block font-sans">Revenue</span>
                        </div>
                      </button>
                    );
                  })
            ...
            <div className="lg:col-span-8 bg-white border border-slate-100 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-4">
              {(() => {
                const rawHistory = currentSalesperson ? (salespersonSalesHistory[currentSalesperson] || []) : [];
                const currentSalespersonStats = {
                  totalQty: rawHistory.reduce((sum, item) => sum + item.quantity, 0),
                  totalRevenue: rawHistory.reduce((sum, item) => sum + item.totalPrice, 0)
                };
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-1">
                          <span className="text-xs text-slate-650 font-bold">
                            Total Items Sold: <span className="text-indigo-600 font-mono font-extrabold">{currentSalespersonStats.totalQty} units</span>
                          </span>
                          <span className="text-xs text-slate-655 font-bold">
                            Total Checkout Value: <span className="text-emerald-600 font-mono font-extrabold">₹{currentSalespersonStats.totalRevenue.toLocaleString()}</span>
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="relative w-full sm:w-56 shrink-0">
                        ...
                        <input
                          ...
                          onChange={(e) => setSoldProductsSearchQuery(e.target.value)}
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="bg-slate-50/75 border-b border-slate-100 text-slate-400 font-extrabold uppercase tracking-wider text-[9px]">
                            ...
                            <th className="p-2.5 text-right font-bold text-indigo-600">Total Price</th>
                          ...
                          {filteredSoldProducts.length === 0 ? (
                            <tr>
                              <td colSpan={7} className="p-12 text-center text-slate-400">
                                ...
                                <p className="font-semibold text-xs">No checkout entries registered for this salesperson.</p>
                                <p className="text-[10px] text-slate-400 mt-0.5 font-normal">Checkout items with this salesperson selected at POS billing to update automatically.</p>
                              </td>
                            </tr>
                          ) : (
                            filteredSoldProducts.map(item => (
                              <tr key={item.id} className="hover:bg-slate-50/45 transition-colors">
                                <td className="p-2.5">
                                  <p className="font-mono font-bold text-slate-800">{item.invoiceNo}</p>
                                  <p className="text-[9px] text-slate-400 font-mono mt-0.5">{item.date}</p>
                                </td>
                                <td className="p-2.5 font-bold text-slate-700 max-w-[160px] truncate animate-fade-in" title={item.productName}>
                                  {item.productName}
                                </td>
                                <td className="p-2.5">
                                  <div className="flex items-center gap-1 flex-wrap">
                                    <span className="bg-slate-100 px-1.5 py-0.2 rounded text-[8px] font-bold text-slate-500 uppercase">{item.size}</span>
                                    <span className="bg-indigo-50 px-1.5 py-0.2 rounded text-[8px] font-bold text-indigo-600 uppercase">{item.color}</span>
                                    <span className="text-[9px] text-slate-400 font-mono truncate max-w-[80px]" title={item.sku}>{item.sku}</span>
                                  </div>
                                </td>
                                ...
                                <td className="p-2.5 truncate max-w-[100px] font-bold text-slate-650" title={item.customerName}>
                                  {item.customerName}
                                </td>
                              </tr>
                            ))
      ...
      {activeTab === 'rules' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs lg:col-span-2 space-y-6">
            <div>
              <h3 className="text-base font-extrabold text-slate-800">Compensation Rules Library</h3>
              <p className="text-xs text-slate-400 mt-0.5">Define automated rules that trigger bonuses and incentive disbursements at billing checkout.</p>
            </div>

            <div className="grid grid-cols-1 gap-3">
              {commissionRulesList.map((rule, idx) => (
                <div key={idx} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 flex justify-between items-center hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-mono">System Code: COM-RULE-{100 + idx}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => {
                      setCommissionRulesList(prev => prev.filter((_, ri) => ri !== idx));
                      onAddNotification('Rule Suspended', 'Commission rule revoked from calculation engine.', 'warning');
                    }}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs space-y-5">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Create Auto Commission Rule</h4>
            
            <form onSubmit={(e) => {
              e.preventDefault();
              const ruleInput = (e.target as any).ruleName.value;
              if (!ruleInput) return;
              setCommissionRulesList([...commissionRulesList, ruleInput]);
              onAddNotification('Commission Rule Live', 'New bonus rule has been saved to checkout engine.', 'success');
              (e.target as any).reset();
            }} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Incentive Description Rule *</label>
                <input 
                  type="text" 
                  name="ruleName" 
              ...
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Calculation Type</label>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xs lg:col-span-8">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Settlement & Disbursement Ledger</h3>
                <p className="text-[10px] text-slate-400 font-medium">Reconciled payments issued outwards to partners and staff.</p>
              </div>
              <button 
                onClick={() => handleExportSimulated('PDF', 'Settlements Ledger')}
                ...
                <tbody className="divide-y divide-slate-100">
                  {settlementHistory.map(item => (
                    <tr key={item.id} className="hover:bg-slate-50/40">
                      <td className="p-3.5 font-mono text-slate-600">{item.date}</td>
                      <td className="p-3.5">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                          item.type === 'Marketplace' ? 'bg-amber-50 text-amber-600' :
                          item.type === 'Influencer' ? 'bg-emerald-50 text-emerald-600' :
                          'bg-indigo-50 text-indigo-600'
                        }`}>
                          {item.type}
                        </span>
                      </td>
                      <td className="p-3.5 font-semibold text-slate-800">{item.recipient}</td>
                      <td className="p-3.5 font-mono text-slate-500 font-bold">{item.referenceNo}</td>
                      <td className="p-3.5 text-right font-mono font-extrabold text-slate-900">
                        ₹{item.amount.toLocaleString()}
                      </td>
                      <td className="p-3.5 font-medium text-slate-600">{item.paymentMethod}</td>
                      <td className="p-3.5 text-center">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs lg:col-span-4 space-y-4">
            <div className="border-b border-slate-100 pb-3 flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">System Audit Timeline</h4>
            </div>
            ...
            <div className="space-y-4 relative pl-3 before:absolute before:left-0.5 before:top-1.5 before:bottom-1 before:w-0.5 before:bg-slate-100">
              {auditLogs.map(log => (
                <div key={log.id} className="text-xs space-y-1 relative">
                  ...
                  <p className="font-extrabold text-slate-700 text-[11px] leading-tight">{log.action}</p>
                  <p className="text-[10px] text-slate-500 leading-relaxed">{log.details}</p>
                  <span className="text-[9px] font-bold text-indigo-600 block bg-slate-50 px-1.5 py-0.2 rounded w-fit">{log.module}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {!['salesperson', 'rules', 'history', 'marketplace', 'influencer'].includes(activeTab) && (
        <div className="space-y-6 animate-fade-in">
          {(() => {
            const currentCat = customCategories.find(c => c.id === activeTab) || {
              name: 'Custom Category',
              description: 'Manage specialized partner ledgers, commissions, and B2B distributions.',
              commissionType: 'Percentage',
              defaultValue: 10
            };

            const partners = categoryPartners.filter(p => p.categoryId === activeTab);
            const records = categoryRecords.filter(r => r.categoryId === activeTab);
            ...
            const pendingSettlement = records.filter(r => r.status === 'Pending').reduce((sum, r) => sum + r.commissionAmount, 0);
                ...
                <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 text-white relative overflow-hidden shadow-md">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
                    <div className="space-y-1">
                      ...
                      <h3 className="text-lg font-extrabold tracking-tight">{currentCat.name}</h3>
                      <p className="text-xs text-slate-400 max-w-xl">{currentCat.description}</p>
                    </div>
                      ...
                      <button
                        onClick={() => {
                          if (partners.length === 0) {
                            onAddNotification('Action Denied', 'Please enroll at least one partner first.', 'warning');
                            return;
                          }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-800 relative z-10">
                    <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Enrolled Partners</p>
                      <p className="text-lg font-bold mt-1 font-mono text-white">{partners.length} active</p>
                    </div>
                    <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Total Volume Logged</p>
                      <p className="text-lg font-bold mt-1 font-mono text-emerald-400">₹{totalRevenue.toLocaleString()}</p>
                    </div>
                    <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Commission Incurred</p>
                      <p className="text-lg font-bold mt-1 font-mono text-indigo-400">₹{totalCommission.toLocaleString()}</p>
                    </div>
                    <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Pending Payouts</p>
                      <p className="text-lg font-bold mt-1 font-mono text-amber-400">₹{pendingSettlement.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xs space-y-4 p-5">
                    <div className="flex justify-between items-center">
                      <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wide">Enrolled Partners Directory</h4>
                      <span className="text-[10px] text-slate-400">Category Basis: <span className="font-bold text-slate-600">{currentCat.defaultValue}{currentCat.commissionType === 'Percentage' ? '%' : ' ₹'}</span></span>
                    </div>

                    {partners.length === 0 ? (
                      <div className="py-16 text-center text-slate-400 space-y-2">
                        <Users className="w-10 h-10 text-slate-300 mx-auto stroke-1" />
                        <p className="text-xs font-bold">No partners enrolled yet.</p>
                        <p className="text-[10px]">Enroll your first distributor, agent, or custom channel partner to start tracking.</p>
                      </div>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse text-xs">
                          ...
                          <tbody className="divide-y divide-slate-50">
                            {partners.map(p => (
                              <tr key={p.id} className="hover:bg-slate-50/50">
                                ...
                                <td className="p-3 text-[10px] text-slate-400">{p.phone}<br />{p.email}</td>
                                <td className="p-3 text-right font-bold text-slate-700 font-mono">₹{p.revenue.toLocaleString()}</td>
                                <td className="p-3 text-right font-bold text-indigo-600 font-mono">₹{p.commissionEarned.toLocaleString()}</td>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs space-y-4">
                    <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wide">Recent Commission Accruals</h4>
                    {records.length === 0 ? (
                      <div className="py-12 text-center text-slate-400 text-xs">
                        <p className="font-semibold">No commission records yet.</p>
                        <p className="text-[10px] text-slate-400">Click "Log Sale" to generate a dynamic commission record.</p>
                      </div>
                    ) : (
                      <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                        {records.map(rec => (
                          <div key={rec.id} className="border border-slate-100 bg-slate-50/40 rounded-xl p-3 space-y-2 hover:bg-slate-50 transition-all">
                            <div className="flex justify-between items-center border-b border-slate-100/60 pb-1.5">
                              <span className="font-extrabold text-slate-800 text-[11px]">{rec.partnerName}</span>
                              <span className="text-[9px] text-slate-400 font-mono">{rec.date}</span>
                            </div>
                            <div className="flex justify-between items-center text-xs font-mono">
                              <div>
                                <span className="text-[10px] text-slate-400 block">Logged Sale</span>
                                <span className="font-bold text-slate-700">₹{rec.saleAmount.toLocaleString()}</span>
                              </div>
                              <div className="text-right">
                                <span className="text-[10px] text-slate-400 block">Commission Accrued</span>
                                <span className="font-extrabold text-indigo-600">₹{rec.commissionAmount.toLocaleString()}</span>
                              </div>
                            </div>
                            ...
                            <div className="pt-2 border-t border-dashed border-slate-200/60 flex items-center justify-between">
                              ...
                              {rec.status === 'Pending' ? (
                                <button
                                  type="button"
                                  onClick={() => handleApproveDynamicSettlement(rec.id)}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full text-slate-700 space-y-5 animate-scale-up">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Configure Commission Channel</h3>
              <button 
                type="button"
            ...
            <form onSubmit={handleCreateCategorySubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Category Name *</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Default Rate *</label>
                  <input 
                    type="number" 
                    value={newCatValue}
      ...
      {showAddPartnerModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full text-slate-700 space-y-5 animate-scale-up">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Enroll Channel Partner</h3>
              <button 
                type="button"
            ...
            <form onSubmit={handleAddDynamicPartner} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Partner/Agency Name *</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Referral / Promo Code *</label>
                  <input 
                    type="text" 
      ...
      {showAddRecordModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full text-slate-700 space-y-5 animate-scale-up">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Log Channel Sale</h3>
              <button 
                type="button"
            ...
            <form onSubmit={handleAddDynamicRecord} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Select Channel Partner *</label>
                <select
                ...
                >
                  ...
                  {categoryPartners.filter(p => p.categoryId === activeTab).map(p => (
                    <option key={p.id} value={p.id}>{p.name} ({p.referenceNo})</option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Gross Sale Amount (₹) *</label>
                  <input 
                    type="number" 
                    value={recordSaleAmt || ''}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Transaction Invoice Ref</label>
                  <input 
                    type="text" 
      ...
      {showAddMarketplaceModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full text-slate-700 space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Record Marketplace Sale Transaction</h3>
              <button 
            ...
            <form onSubmit={handleAddMarketplaceOrder} className="space-y-3.5 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Marketplace Platform *</label>
                  <select 
                    value={mpMarketplace}
                    onChange={(e) => setMpMarketplace(e.target.value as any)}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Order Identifier *</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Invoice Reference</label>
                  <input 
                    type="text" 
              ...
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Apparel / Product Sold *</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Price Sold (₹) *</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Commission %</label>
                  <input 
                    type="number" 
                    value={mpCommissionPercent} 
                    onChange={(e) => setMpCommissionPercent(Number(e.target.value))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5 font-mono font-bold text-slate-800" 
                  />
                </div>

                <div>
                  <label className="block text-slate-500 font-semibold mb-1">GST/Tax %</label>
                  <select 
                    value={mpTax} 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Shipping Cost (₹)</label>
                  <input 
                    type="number" 
                    value={mpShipping} 
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Packaging Cost (₹)</label>
                  <input 
                    type="number" 
                    value={mpPackaging} 
              ...
              <div className="p-3 bg-indigo-50/50 border border-indigo-100 rounded-2xl text-[11px] space-y-1">
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Expected Net Payout</span>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Platform Commission:</span>
                  <span className="font-mono text-red-600">-₹{Math.round((mpSellingPrice * mpCommissionPercent) / 100)}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Tax deductions (GST):</span>
                  <span className="font-mono text-red-600">-₹{Math.round(mpSellingPrice * (mpTax / 100))}</span>
                </div>
                <div className="flex justify-between items-center border-t border-indigo-100 pt-1 text-slate-800 font-bold text-xs">
                  ...
                  <span className="font-mono text-indigo-700 text-sm">
                    ₹{Math.max(0, mpSellingPrice - Math.round((mpSellingPrice * mpCommissionPercent) / 100) - mpShipping - mpPackaging - Math.round(mpSellingPrice * (mpTax / 100)))}
                  </span>
                </div>
              </div>
      ...
      {showAddInfluencerModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 max-w-md w-full text-slate-700 space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Recruit Brand Influencer Partner</h3>
              <button 
            ...
            <form onSubmit={handleAddInfluencer} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Influencer / Affiliate Name *</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Social Platform *</label>
                  <select 
                    value={infPlatform}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Platform ID/Handle *</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Promo Referral Code *</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Followers Count</label>
                  <input 
                    type="number" 
                    value={infFollowers} 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Commission Type</label>
                  <select 
                    value={infCommissionType}
                    onChange={(e) => setInfCommissionType(e.target.value as any)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 font-semibold text-slate-700 outline-none"
                  >
                    <option value="Percentage">Percentage %</option>
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Commission Rate Value *</label>
                  <input 
                    type="number" 
                    value={infCommissionValue} 
                    onChange={(e) => setInfCommissionValue(Number(e.target.value))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 font-mono font-bold text-slate-800 outline-none"
                    required 
                  />
                </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full text-center space-y-4 shadow-xl">
            ...
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Confirm Record Deletion</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Are you sure you want to permanently delete this commission ledger entry? This operation is irreversible and will affect balance accounting metrics.
            </p>
            <div className="flex gap-2 justify-center">
              ...
              <button 
                onClick={() => handleDeleteItem(showDeleteConfirm.type, showDeleteConfirm.id)}
