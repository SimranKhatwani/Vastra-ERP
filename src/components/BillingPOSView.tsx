...
interface BillingPOSViewProps {
  ...
  onUpdateCustomerBalance: (customerId: string, amount: number) => void;
  onAddNotification: (title: string, msg: string, type: 'info' | 'warning' | 'success' | 'danger') => void;
  quickArticulateItem: any;
  clearQuickArticulateItem: () => void;
}
...
export const BillingPOSView: React.FC<BillingPOSViewProps> = ({
  ...
  const [cashierId, setCashierId] = useState<string>('e-2'); // default cashier
  const [salespersonName, setSalespersonName] = useState<string>('');
  const [rightColumnTab, setRightColumnTab] = useState<'catalog' | 'history'>('catalog');
  ...
  // Payments
  const [paymentMethod, setPaymentMethod] = useState<'Cash' | 'Card' | 'UPI' | 'Wallet' | 'Credit' | 'Split'>('Cash');
  const [splitCash, setSplitCash] = useState<number>(0);
  const [splitCard, setSplitCard] = useState<number>(0);
  const [splitUPI, setSplitUPI] = useState<number>(0);

  // Active view states
  const [activePOSMode, setActivePOSMode] = useState<'billing' | 'history' | 'returns' | 'quotations' | 'orders' | 'credit_notes' | 'debit_notes'>('billing');
  ...
  const [quotations, setQuotations] = useState<any[]>([
    { id: 'q-1', quoteNo: 'QTN-2026-001', customerName: 'Ramesh Kumar', date: '2026-06-25', total: 4500, status: 'Draft', items: [{ name: 'Raymond Executive Linen Shirt - White', quantity: 2, price: 1500, totalPrice: 3000 }] },
    { id: 'q-2', quoteNo: 'QTN-2026-002', customerName: 'Sushma Swaraj', date: '2026-06-27', total: 12500, status: 'Approved', items: [{ name: 'Biba Festive Floral Saree - Red Silk', quantity: 1, price: 8500, totalPrice: 8500 }] }
  ]);
  ...
  const [salesOrders, setSalesOrders] = useState<any[]>([
    { id: 'so-1', orderNo: 'SO-2026-101', customerName: 'Ramesh Kumar', date: '2026-06-26', total: 9200, status: 'Pending', itemsCount: 3 },
    { id: 'so-2', orderNo: 'SO-2026-102', customerName: 'Aman Deep', date: '2026-06-28', total: 18500, status: 'Dispatched', itemsCount: 5 }
  ]);
  ...
  const [creditNotes, setCreditNotes] = useState<any[]>([
    { id: 'cn-1', noteNo: 'CN-2026-01', invoiceNo: 'INV-20260499', customerName: 'Ramesh Kumar', amount: 1500, reason: 'Damaged Collar seam', date: '2026-06-27' }
  ]);
  ...
  const [debitNotes, setDebitNotes] = useState<any[]>([
    { id: 'dn-1', noteNo: 'DN-2026-01', invoiceNo: 'INV-20260498', customerName: 'Sushma Swaraj', amount: 800, reason: 'Express delivery charges', date: '2026-06-26' }
  ]);
  ...
  const [debitReason, setDebitReason] = useState('Extra custom tailoring adjustments');
  ...
  const [articulationProduct, setArticulationProduct] = useState<Product | null>(null);
  const [selectedVariant, setSelectedVariant] = useState<Product | null>(null);
  const [articulationQty, setArticulationQty] = useState<number>(1);
  const [articulationSearch, setArticulationSearch] = useState<string>('');
  const [spreadsheetQuantities, setSpreadsheetQuantities] = useState<Record<string, number>>({});
  const [activeCellId, setActiveCellId] = useState<{ row: number; col: string } | null>({ row: 1, col: 'A' });

  // Memoized variant matrices for the active product
  const articulationVariants = React.useMemo(() => {
    if (!articulationProduct) return [];
    return products.filter(p => p.brand === articulationProduct.brand && p.category === articulationProduct.category);
  ...
  const handleAddProductToCartWithQty = (prod: Product, qty: number) => {
    if (prod.stock <= 0) {
      onAddNotification('POS Warning', `${prod.name} is currently out of stock.`, 'warning');
    }

    setCart(prev => {
      const existingIdx = prev.findIndex(item => item.productId === prod.id && !item.isCustom);
      if (existingIdx > -1) {
        const updated = [...prev];
        const newQty = updated[existingIdx].quantity + qty;
        const sub = prod.sellingPrice * newQty;
        const discountAmt = Math.floor(sub * (updated[existingIdx].discount / 100));
        const itemGst = Math.floor((sub - discountAmt) * (prod.gstPercent / 100));
  ...
  useEffect(() => {
    if (quickArticulateItem) {
      ...
      onAddNotification('POS Terminal Feed', 'Articulated Custom Garment added to POS basket.', 'success');
      clearQuickArticulateItem();
    }
  }, [quickArticulateItem]);
  ...
  // Barcode quick simulated lookup
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    if (found) {
      ...
      onAddNotification('Barcode Scanner', `Scanned and added: ${found.name}`, 'success');
      setBarcodeInput('');
    } else {
      onAddNotification('Barcode Error', `No product found matching code: "${barcodeInput}"`, 'danger');
    }
  };
  ...
  const handleAdjustItemDiscount = (idx: number, discountPct: number) => {
    setCart(prev => {
      const updated = [...prev];
      const item = updated[idx];
  ...
  const calculateCartTotals = () => {
    ...
    if (couponCode === 'WINTER20') {
      couponDiscount = Math.floor(subTotal * 0.20);
  ...
  const { subTotal, discountTotal, couponDiscount, gstTotal, grandTotal } = calculateCartTotals();

  // Handle checkout
  const handleCheckoutSubmit = () => {
    if (cart.length === 0) {
      onAddNotification('POS Checkout Failed', 'Cannot compile an empty cart.', 'danger');
      return;
    }
    ...
    const newInvoice: Invoice = {
      ...
      ] as { method: 'Cash' | 'Card' | 'UPI' | 'Wallet'; amount: number }[]).filter(s => s.amount > 0) : undefined,
    ...
    if (paymentMethod === 'Credit') {
      ...
      onAddNotification('Credit Balance Logged', `₹${grandTotal.toLocaleString()} logged to ${activeCustomer.name}'s credit ledger.`, 'info');
    }
    ...
    onAddNotification('Invoice Compiled Successfully', `Issued receipt ${newInvoice.invoiceNo} for ₹${newInvoice.grandTotal.toLocaleString()}`, 'success');
  };
  ...
  const handleCreateCustomer = (e: React.FormEvent) => {
    ...
    onAddNotification('CRM Engine', 'New Retail Customer profile saved successfully.', 'success');
  };
  ...
  const handleCreateQuotation = (e: React.FormEvent) => {
    ...
    const newQuote = {
      ...
      total,
      status: 'Approved',
      items: [{ name: targetP.name, quantity: quoteQty, price: targetP.sellingPrice, totalPrice: total }]
    };

    setQuotations(prev => [...prev, newQuote]);
    onAddNotification('Quotation Compiled', `Quote estimate generated for ${quoteCustName}. Value: ₹${total.toLocaleString()}`, 'success');
  ...
  const handleCreateSalesOrder = (e: React.FormEvent) => {
    ...
    onAddNotification('Sales Order Registered', `Bulk commercial contract registered for ${orderCustName} for ${orderQty} units.`, 'success');
  ...
  const handleCreateCreditNote = (e: React.FormEvent) => {
    ...
    onAddNotification('Credit Note Issued', `Credited ₹${creditAmt.toLocaleString()} to ${targetC.name}'s wallet ledger.`, 'success');
  ...
  const handleCreateDebitNote = (e: React.FormEvent) => {
    ...
    onAddNotification('Debit Note Levied', `Charged ₹${debitAmt.toLocaleString()} to ${targetC.name}'s balance account. Reason: ${debitReason}`, 'success');
  ...
  const handleSubmitReturn = () => {
    ...
    if (returnedItemIds.length === 0) {
      onAddNotification('Return Wizard', 'No items selected to issue a refund or exchange.', 'warning');
      return;
    }
    ...
    const match = invoices.find(inv => inv.id === selectedInvoiceForReturn.id);
    if (match) {
      match.status = 'Returned';
    }

    onAddNotification('Return Approved', `Returned items from ${selectedInvoiceForReturn.invoiceNo}. Credited ₹${refundTotal.toLocaleString()} to customer's account balance.`, 'success');
  ...
  const handleDownloadReceiptHTML = (invoice: Invoice) => {
    const cashierName = employees.find(e => e.id === invoice.employeeId)?.name || invoice.employeeName;
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    onAddNotification('File Downloader', `HTML Invoice ${invoice.invoiceNo} successfully generated & downloaded.`, 'success');
  };

  const handleWhatsAppShare = (invoice: Invoice) => {
    const itemsText = invoice.items.map(item => `- ${item.name} (${item.size}/${item.color}) x ${item.quantity} = ₹${item.totalPrice}`).join('\n');
    const msg = `*VASTRA ERP - INVOICE GENERATED*\n\n` +
    ...
    onAddNotification('WhatsApp API Relay', 'Redirecting to WhatsApp to send invoice...', 'success');
  };
    ...
    <div className="space-y-6 animate-fade-in pb-12" id="billing-pos-root">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {employees.filter(e => e.role === 'Admin' || e.role === 'Cashier' || e.role === 'Manager').map(e => (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 lg:col-span-5 space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Retail Customer Profile</label>
                <button 
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  className="w-full text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                >
                  {customers.map(c => (
                    <option key={c.id} value={c.id}>
                      {c.name} ({c.phone}) - {c.membership} Tier
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex justify-between items-center bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-[11px] font-mono">
                <div>
                  ...
                  <span className="text-emerald-600 font-bold">₹{activeCustomer.walletBalance}</span>
                </div>
                <div>
                  <span className="text-slate-400">Loyalty Points:</span>{' '}
                  <span className="text-violet-600 font-bold">{activeCustomer.loyaltyPoints} PTS</span>
                </div>
                {activeCustomer.outstandingBalance > 0 && (
                  <div>
                    <span className="text-slate-400">Debt:</span>{' '}
                    <span className="text-red-500 font-bold">₹{activeCustomer.outstandingBalance}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Garment Basket ({cart.length})</label>
                {cart.length > 0 && (
                  <button onClick={() => setCart([])} className="text-xs text-red-500 hover:underline flex items-center gap-1 cursor-pointer">
              ...
              {cart.length === 0 ? (
                <div className="border border-dashed border-slate-200 rounded-xl py-10 flex flex-col items-center justify-center text-slate-400 space-y-2">
                  <Barcode className="w-10 h-10 text-slate-300 stroke-1" />
                  <span className="text-xs">Scan barcodes or select garments to build bill</span>
                </div>
              ) : (
                      ...
                      <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-slate-400 font-mono">
                            {item.sku} | Size: {item.size} | Color: {item.color} {item.isCustom && <span className="bg-violet-100 text-violet-700 px-1 py-0.2 rounded font-sans font-bold uppercase text-[8px]">Customized</span>}
                          </p>
                        </div>
                        <span className="text-xs font-bold text-slate-800 shrink-0">₹{item.totalPrice.toLocaleString()}</span>
                      </div>

                      <div className="flex items-center justify-between">
                        {/* Qty Adjustment */}
                        <div className="flex items-center border border-slate-200 rounded-lg">
                          <button onClick={() => handleAdjustQty(idx, -1)} className="p-1 text-slate-500 hover:bg-slate-100 rounded-l-lg cursor-pointer">
                            <Minus className="w-3.5 h-3.5" />
                          </button>
                          <span className="px-2.5 text-xs font-mono font-bold text-slate-700">{item.quantity}</span>
                          <button onClick={() => handleAdjustQty(idx, 1)} className="p-1 text-slate-500 hover:bg-slate-100 rounded-r-lg cursor-pointer">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <select 
                            value={item.discount} 
                            onChange={(e) => handleAdjustItemDiscount(idx, Number(e.target.value))}
                            className="text-[11px] font-mono text-slate-600 bg-white border border-slate-200 rounded px-1 py-0.5 focus:outline-none"
                          >
            ...
            {/* Coupons & Flat Discounts */}
            <div className="grid grid-cols-2 gap-3 border-t border-slate-100 pt-4">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Coupon Campaign</label>
                <div className="relative">
                  <input 
                    ...
                    className="w-full pl-2 pr-6 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-500 uppercase font-mono"
                  />
                  {couponCode && (
                ...
                {['WINTER20', 'LOYALTY50', 'FESTIVE15'].includes(couponCode) && (
                  <span className="text-[9px] text-emerald-600 font-semibold mt-0.5 block">✓ Coupon Active</span>
                )}
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Flat Discount (₹)</label>
                <input 
                  type="number" 
                  placeholder="0.00" 
                  value={flatDiscount || ''} 
                  onChange={(e) => setFlatDiscount(Math.max(0, Number(e.target.value)))}
                  className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono font-bold focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            </div>

            {/* Tax / Total breakdown */}
            <div className="space-y-1.5 bg-slate-50 border border-slate-100 rounded-2xl p-4 text-xs">
              ...
              <div className="flex justify-between text-base font-bold text-slate-800 border-t border-slate-200/60 pt-2">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Terminal Payment Route</label>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  { id: 'Credit', label: 'Credit Sale', icon: FileText },
                  { id: 'Split', label: 'Split Pay', icon: Percent }
                ].map(p => {
                  const Icon = p.icon;
                  return (
                    <button 
            ...
            {paymentMethod === 'Split' && (
              <div className="bg-indigo-50/60 border border-indigo-100 rounded-xl p-3 space-y-2 text-xs">
                <span className="font-semibold text-slate-700 block">Allocate Split Streams (Total: ₹{grandTotal.toLocaleString()})</span>
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <span className="text-[9px] text-slate-400 font-mono">Cash Part</span>
                    <input type="number" value={splitCash || ''} onChange={(e) => setSplitCash(Number(e.target.value))} className="w-full bg-white border border-slate-200 rounded px-1.5 py-1 font-mono text-xs" />
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 font-mono">Card Part</span>
                    <input type="number" value={splitCard || ''} onChange={(e) => setSplitCard(Number(e.target.value))} className="w-full bg-white border border-slate-200 rounded px-1.5 py-1 font-mono text-xs" />
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 font-mono">UPI Part</span>
                    <input type="number" value={splitUPI || ''} onChange={(e) => setSplitUPI(Number(e.target.value))} className="w-full bg-white border border-slate-200 rounded px-1.5 py-1 font-mono text-xs" />
                  </div>
                </div>
                {splitCash + splitCard + splitUPI !== grandTotal && (
                  <div className="flex items-center gap-1 text-[10px] text-amber-600">
                    <AlertCircle className="w-3.5 h-3.5" />
                    <span>Allocated: ₹{(splitCash + splitCard + splitUPI).toLocaleString()} (Mismatch: ₹{Math.abs(grandTotal - (splitCash + splitCard + splitUPI)).toLocaleString()})</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="space-y-1 bg-slate-50 border border-slate-100 rounded-xl p-3">
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block">Salesperson / Staff</label>
              <input 
                type="text" 
            ...
            <button 
              ...
              className={`w-full py-3 rounded-xl text-xs font-bold tracking-wider uppercase transition-all shadow-md cursor-pointer ${cart.length === 0 ? 'bg-slate-100 text-slate-400 shadow-none cursor-not-allowed' : 'bg-gradient-to-r from-indigo-600 to-indigo-700 text-white hover:brightness-105'}`}
            >
              Generate POS Invoice (Print/SMS)
            </button>
          </div>
          ...
          <div className="space-y-5 lg:col-span-7">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
              <form onSubmit={handleBarcodeSubmit} className="flex gap-2 items-center">
                ...
                <input 
                  type="text" 
                  placeholder="Simulate laser scan (Enter Barcode or SKU, e.g. 890100000001)..." 
                  value={barcodeInput}
                  onChange={(e) => setBarcodeInput(e.target.value)}
                  className="w-full bg-slate-50 text-xs font-semibold px-3 py-2 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono"
                />
                <button type="submit" className="bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-semibold hover:bg-slate-800 transition-all cursor-pointer">
                  Scan
                </button>
              </form>
            </div>

            {/* Right Column Header / Tab Switcher */}
            <div className="flex bg-slate-100/70 p-1 rounded-xl gap-1">
              <button
                type="button"
                onClick={() => setRightColumnTab('catalog')}
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${rightColumnTab === 'catalog' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                <Grid className="w-4 h-4 text-indigo-600" />
                <span>Bespoke Apparel Catalog</span>
              </button>
              <button
                type="button"
                onClick={() => setRightColumnTab('history')}
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${rightColumnTab === 'history' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-800 relative'}`}
              >
                <Clock className="w-4 h-4 text-indigo-600" />
                <span>Customer Purchase History</span>
                {invoices.filter(inv => inv.customerId === selectedCustomerId || inv.customerPhone === activeCustomer.phone).length > 0 && (
                  <span className="bg-indigo-600 text-white font-mono text-[9px] px-1.5 py-0.5 rounded-full font-bold ml-1">
                    {invoices.filter(inv => inv.customerId === selectedCustomerId || inv.customerPhone === activeCustomer.phone).length}
                  </span>
                )}
              </button>
            </div>
            ...
            {rightColumnTab === 'catalog' && (
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide">Bespoke Apparel Catalog</h3>
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-2 scrollbar-none">
                {uniqueCategories.slice(0, 10).map((cat, idx) => (
                  <button 
                    key={idx}
                    onClick={() => setSelectedCategoryFilter(cat)}
                    className={`px-3 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-wide transition-all shrink-0 cursor-pointer ${selectedCategoryFilter === cat ? 'bg-slate-800 text-white' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                  >
                    {cat}
                  </button>
                ))}
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        ...
                        onAddNotification('POS Billing', `Added ${p.name} to POS cart.`, 'success');
                      }}
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex justify-between items-start gap-1">
                          <span className="text-[8px] bg-slate-100 text-slate-600 px-1.5 py-0.2 rounded font-semibold font-mono uppercase truncate max-w-[80px]">{p.brand}</span>
                          <span className={`text-[8px] px-1 py-0.2 rounded font-bold uppercase ${isLow ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>
                            {p.stock} left
                          </span>
                        </div>
                        <h4 className="text-xs font-semibold text-slate-800 line-clamp-2 min-h-[32px]">{p.name}</h4>
                      </div>

                      <div className="mt-2.5 flex items-end justify-between">
                        <div className="space-y-0.5">
                          <span className="text-[10px] text-slate-400 line-through font-mono">₹{p.mrp}</span>
                          <p className="text-xs font-bold text-indigo-600 font-mono">₹{p.sellingPrice}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              onAddNotification('Articulation Desk', `Opening spreadsheet matrix for: ${p.brand} ${p.category}`, 'info');
                            }}
          ...
          {rightColumnTab === 'history' && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4 animate-fade-in">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div>
                  <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wide">Historical Purchases</h3>
                  <p className="text-[10px] text-slate-400 mt-0.5">Showing previous receipts for <span className="text-indigo-600 font-bold">{activeCustomer.name}</span></p>
                </div>
              ...
              {(() => {
                const customerInvoices = invoices.filter(inv => 
                  ...
                  inv.customerName.toLowerCase() === activeCustomer.name.toLowerCase()
                );
                ...
                if (customerInvoices.length === 0) {
                  return (
                    <div className="py-12 text-center text-slate-400 space-y-2">
                      ...
                      <p className="text-xs font-semibold">No earlier bills found for this customer.</p>
                      <p className="text-[10px]">When they complete their first transaction, their purchase log will register here.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                    {customerInvoices.map((inv, index) => (
                      <div key={index} className="border border-slate-100 bg-slate-50/40 hover:bg-slate-50 rounded-xl p-3.5 space-y-3 transition-all">
                        <div className="flex justify-between items-center text-xs font-semibold border-b border-slate-100/60 pb-2">
                          ...
                          <span className="text-slate-400 font-mono text-[10px]">{inv.date}</span>
                        </div>
                          ...
                          {inv.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between text-xs text-slate-600">
                              <div>
                                ...
                                <span className="text-[10px] text-slate-400 block font-mono">Size: {item.size} | Color: {item.color} | Qty: {item.quantity}</span>
                              </div>
                              <span className="font-bold font-mono text-slate-700">₹{item.totalPrice.toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="border-t border-slate-100/60 pt-2 flex justify-between items-center">
                          <div className="text-[10px] text-slate-400 space-y-0.5">
                            {inv.salespersonName && (
                              <p>Salesperson: <span className="font-bold text-slate-600">{inv.salespersonName}</span></p>
                            )}
                            <p>Pay Mode: <span className="font-bold text-slate-600">{inv.paymentMethod}</span></p>
                          </div>
                          <div className="text-right">
                            <span className="text-[9px] text-slate-400 block">Grand Total</span>
                            <span className="font-extrabold text-xs text-slate-800 font-mono">₹{inv.grandTotal.toLocaleString()}</span>
                          </div>
                        </div>
                            ...
                            onClick={() => {
                              ...
                              inv.items.forEach(item => {
                                setCart(prev => {
                                  ...
                                  if (existing) {
                                    return prev.map(i => i.productId === item.productId && i.size === item.size && i.color === item.color ? { ...i, quantity: i.quantity + item.quantity, totalPrice: i.totalPrice + item.totalPrice } : i);
                                  }
                                  return [...prev, { ...item }];
                                });
                              });
                              onAddNotification('POS Terminal Basket', `Copied items from invoice ${inv.invoiceNo} into active basket.`, 'success');
                            }}
      ...
      {activePOSMode === 'history' && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Historical Billing Logs</h3>
              <p className="text-xs text-slate-400">Total processed transactions: {invoices.length} invoices</p>
            </div>
            <div className="flex gap-2">
              <input 
                type="text" 
              ...
              <tbody className="divide-y divide-slate-100 text-slate-600">
                {invoices.slice(0, 12).map((inv, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3 font-mono">{(inv.items || []).reduce((sum, i) => sum + i.quantity, 0)} pcs</td>
                    <td className="p-3 font-bold font-mono">₹{inv.grandTotal.toLocaleString()}</td>
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${inv.paymentMethod === 'Cash' ? 'bg-emerald-50 text-emerald-600' : 'bg-indigo-50 text-indigo-600'}`}>
                        {inv.paymentMethod}
                      </span>
                    </td>
                    <td className="p-3">
      ...
      {activePOSMode === 'returns' && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 md:col-span-5 space-y-4">
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Lookup Previous Invoice</h4>
            <div className="space-y-3">
              <div>
                <span className="text-[10px] text-slate-400 block mb-1 font-mono">Invoice Number</span>
                <select 
                  onChange={(e) => {
                    const match = invoices.find(inv => inv.invoiceNo === e.target.value);
                ...
                >
                  ...
                  {invoices.slice(0, 20).map(inv => (
                    <option key={inv.id} value={inv.invoiceNo}>{inv.invoiceNo} - {inv.customerName}</option>
                  ))}
                </select>
              </div>
              ...
              {selectedInvoiceForReturn && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2.5 text-xs">
                  <div className="flex justify-between font-semibold">
                    ...
                    <span className="font-mono">{selectedInvoiceForReturn.date}</span>
                  </div>
                  <div className="flex justify-between font-semibold">
                    <span className="text-slate-400">Grand Total:</span>
                    <span className="font-mono text-indigo-600">₹{selectedInvoiceForReturn.grandTotal.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between font-semibold">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 md:col-span-7 space-y-4">
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono">Select Items to Return / Swap</h4>
            {!selectedInvoiceForReturn ? (
              <div className="py-12 border border-dashed border-slate-200 rounded-xl text-center text-slate-400 text-xs">
                Please search/select an invoice on the left to review return eligibility.
              </div>
            ) : (
              <div className="space-y-4">
                      ...
                      <div key={idx} className="py-2 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <input 
                            ...
                            onChange={() => {
                              setReturnedItemIds(prev => 
                                isChecked ? prev.filter(id => id !== item.productId) : [...prev, item.productId]
                              );
                            }}
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-mono">Size: {item.size} | Color: {item.color} | Qty: {item.quantity}</p>
                          </div>
                        </div>
                        <span className="text-xs font-bold font-mono text-slate-800">₹{item.totalPrice.toLocaleString()}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="flex justify-between border-t border-slate-100 pt-3 text-xs">
                  <span className="font-semibold text-slate-500">Refund Credit Estimated:</span>
      ...
      {activePOSMode === 'quotations' && (
        <div className="space-y-4 text-xs animate-fade-in">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Garment Quotation Builder</h3>
              <p className="text-[11px] text-slate-400">Generate commercial estimates for custom apparel projects.</p>
            </div>
            <button 
              onClick={() => {
                setQuoteCustName('');
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {quotations.map((q, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5 font-mono font-bold text-indigo-600">{q.quoteNo}</td>
                      ...
                      <td className="p-3.5 text-right font-mono font-bold text-slate-800">₹{q.total.toLocaleString()}</td>
                      <td className="p-3.5">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${q.status === 'Approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                          {q.status}
                        </span>
                      </td>
                      <td className="p-3.5 text-center">
                        <button 
                          onClick={() => {
                            ...
                            const quoteItems = q.items.map((it: any) => {
                              const prod = products.find(p => p.name === it.name) || products[0];
                              return {
                                productId: prod.id,
                                name: prod.name,
                                sku: prod.sku,
                            ...
                            setCart(quoteItems);
                            setActivePOSMode('billing');
                            onAddNotification('Converted Quote', `Converted ${q.quoteNo} into POS active checkout session.`, 'success');
                          }}
      ...
      {activePOSMode === 'orders' && (
        <div className="space-y-4 text-xs animate-fade-in">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Bulk Sales Orders Ledger</h3>
              <p className="text-[11px] text-slate-400">Track large custom production runs and franchise orders.</p>
            </div>
            <button 
              onClick={() => {
                setOrderCustName('');
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {salesOrders.map((so, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5 font-mono font-bold text-indigo-600">{so.orderNo}</td>
                      <td className="p-3.5 font-mono">{so.date}</td>
                      <td className="p-3.5 font-bold text-slate-800">{so.customerName}</td>
                      <td className="p-3.5 text-right font-mono">{so.itemsCount} lines</td>
                      <td className="p-3.5 text-right font-mono font-bold text-slate-800">₹{so.total.toLocaleString()}</td>
                      <td className="p-3.5">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                          so.status === 'Dispatched' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                        }`}>
                          {so.status}
                        </span>
                      </td>
                      <td className="p-3.5 text-center">
                        {so.status === 'Pending' ? (
                          <button 
                            onClick={() => {
                              setSalesOrders(prev => prev.map(o => o.id === so.id ? { ...o, status: 'Dispatched' } : o));
                              onAddNotification('Order Shipped', `Sales Order ${so.orderNo} status flagged as DISPATCHED.`, 'success');
                            }}
                          ...
                          <span className="text-slate-400 font-bold text-[10px]">Fulfillment Completed</span>
                        )}
                      </td>
                    </tr>
                  ))}
      ...
      {activePOSMode === 'credit_notes' && (
        <div className="space-y-4 text-xs animate-fade-in">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Credit Notes Ledger</h3>
              <p className="text-[11px] text-slate-400">Issue commercial refunds or pricing waivers credited directly to client wallets.</p>
            </div>
            <button 
              onClick={() => {
                setCreditInvoiceNo('');
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {creditNotes.map((cn, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5 font-mono font-bold text-red-600">{cn.noteNo}</td>
                      <td className="p-3.5 font-mono">{cn.date}</td>
                      <td className="p-3.5 font-mono text-slate-500">{cn.invoiceNo}</td>
                      <td className="p-3.5 font-bold text-slate-800">{cn.customerName}</td>
                      <td className="p-3.5 text-slate-500">{cn.reason}</td>
                      <td className="p-3.5 text-right font-mono font-bold text-emerald-600">₹{cn.amount.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activePOSMode === 'debit_notes' && (
        <div className="space-y-4 text-xs animate-fade-in">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Debit Notes Ledger</h3>
              <p className="text-[11px] text-slate-400">Levy charges, freight costs, or bespoke alterations additions to ledger balance.</p>
            </div>
            <button 
              onClick={() => {
                setDebitInvoiceNo('');
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {debitNotes.map((dn, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5 font-mono font-bold text-indigo-600">{dn.noteNo}</td>
                      <td className="p-3.5 font-mono">{dn.date}</td>
                      <td className="p-3.5 font-mono text-slate-500">{dn.invoiceNo}</td>
                      <td className="p-3.5 font-bold text-slate-800">{dn.customerName}</td>
                      <td className="p-3.5 text-slate-500">{dn.reason}</td>
                      <td className="p-3.5 text-right font-mono font-bold text-slate-800">₹{dn.amount.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {showQuoteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 text-xs animate-fade-in">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 border border-slate-100 shadow-xl">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Create Garment Quotation</h3>
            ...
            <form onSubmit={handleCreateQuotation} className="space-y-4">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Customer / Lead Name</label>
                <input 
                  type="text" 
                  required
                  value={quoteCustName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Select Catalog Item</label>
                  <select 
                    required
                    value={quoteProdId}
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setShowQuoteModal(false)} className="px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-xl font-bold cursor-pointer">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold cursor-pointer hover:bg-slate-800">Generate Estimate</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showOrderModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 text-xs animate-fade-in">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 border border-slate-100 shadow-xl">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Register Bulk Sales Order</h3>
            ...
            <form onSubmit={handleCreateSalesOrder} className="space-y-4">
              <div>
                <label className="block text-slate-500 font-semibold mb-1">B2B Franchise/Client Name</label>
                <input 
                  type="text" 
                  required
                  value={orderCustName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Assigned SKU Product</label>
                  <select 
                    required
                    value={orderProdId}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Production Qty</label>
                  <input 
                    type="number" 
                    required
                    min={1}
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setShowOrderModal(false)} className="px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-xl font-bold cursor-pointer">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold cursor-pointer hover:bg-slate-800">Register Sales Order</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showCreditModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 text-xs animate-fade-in">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 border border-slate-100 shadow-xl">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Issue Credit Note Waiver</h3>
            ...
            <form onSubmit={handleCreateCreditNote} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Beneficiary Customer</label>
                  <select 
                    value={creditCustId}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Linked Invoice #</label>
                  <input 
                    type="text" 
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Credit Note Value (₹)</label>
                  <input 
                    type="number" 
                    required
                    min={1}
                    value={creditAmt}
                    onChange={(e) => setCreditAmt(Math.max(1, Number(e.target.value)))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 font-mono font-bold text-slate-800 outline-none" 
                  />
                </div>
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Waiver Reason</label>
                  <input 
                    type="text" 
                    required
                    value={creditReason}
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setShowCreditModal(false)} className="px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-xl font-bold cursor-pointer">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold cursor-pointer hover:bg-slate-800">Issue Note</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showDebitModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 text-xs animate-fade-in">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 border border-slate-100 shadow-xl">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Issue Debit Surcharge Note</h3>
            ...
            <form onSubmit={handleCreateDebitNote} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Charge Customer Account</label>
                  <select 
                    value={debitCustId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Debit Value (₹)</label>
                  <input 
                    type="number" 
                    required
                    min={1}
                    value={debitAmt}
                    onChange={(e) => setDebitAmt(Math.max(1, Number(e.target.value)))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 font-mono font-bold text-slate-800 outline-none" 
                  />
                </div>
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Surcharge Reason</label>
                  <input 
                    type="text" 
                    required
                    value={debitReason}
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setShowDebitModal(false)} className="px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-xl font-bold cursor-pointer">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold cursor-pointer hover:bg-slate-800">Issue Note</button>
              </div>
            </form>
          </div>
      ...
      {showAddCustomerModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-xl border border-slate-100 animate-scale-up">
            <div className="flex justify-between items-center">
              <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Register New Customer</h4>
            ...
            <form onSubmit={handleCreateCustomer} className="space-y-3 text-xs">
              <div>
                ...
                <input required type="text" placeholder="e.g. Shashi Kapoor" value={newCustName} onChange={(e) => setNewCustName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-xs focus:ring-1 focus:ring-indigo-500 outline-none font-semibold text-slate-800" />
              </div>

              <div>
                <label className="block text-slate-500 mb-1 font-semibold">Contact Mobile *</label>
                <input required type="text" placeholder="e.g. 9876543210" value={newCustPhone} onChange={(e) => setNewCustPhone(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-xs focus:ring-1 focus:ring-indigo-500 outline-none font-semibold text-slate-800" />
              </div>

              <div>
                <label className="block text-slate-500 mb-1 font-semibold">Email Address</label>
                <input type="email" placeholder="e.g. shashi@gmail.com" value={newCustEmail} onChange={(e) => setNewCustEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-xs focus:ring-1 focus:ring-indigo-500 outline-none font-semibold text-slate-800" />
              </div>

              <button type="submit" className="w-full py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 shadow-md cursor-pointer">
      ...
      {articulationProduct && (() => {
        ...
        const spreadsheetFilteredVariants = products.filter(p => p.brand === articulationProduct.brand && p.category === articulationProduct.category).filter(v => {
          if (!articulationSearch) return true;
          const qs = articulationSearch.toLowerCase();
          return v.name.toLowerCase().includes(qs) ||
            ...
            <div className="bg-slate-100 rounded-2xl shadow-2xl border border-slate-300 max-w-6xl w-full flex flex-col overflow-hidden my-4 max-h-[92vh] text-slate-800 animate-scale-up">
              ...
              <div className="bg-[#107c41] text-white px-4 py-2.5 flex items-center justify-between shadow-md shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <div className="leading-none">
                    <span className="text-xs font-bold font-mono tracking-tight text-emerald-100 uppercase block">Excel Inventory Workspace</span>
                    <h3 className="text-sm font-extrabold font-mono">
                      Sku_Matrix_{articulationProduct.brand.replace(/\s+/g, '_')}_{articulationProduct.category.replace(/\s+/g, '_')}.xlsx
                    </h3>
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              <div className="bg-white border-b border-slate-200 px-4 py-1 text-xs text-slate-600 flex gap-4 select-none overflow-x-auto shrink-0 scrollbar-none">
                {['File', 'Edit', 'View', 'Insert', 'Format', 'Data', 'Tools', 'Extensions', 'Help'].map(menu => (
                  <span key={menu} className="hover:bg-slate-100 px-2 py-0.5 rounded cursor-pointer transition-colors font-medium">{menu}</span>
                ))}
              </div>
              ...
              <div className="bg-slate-50 border-b border-slate-200 px-4 py-1.5 flex flex-wrap items-center gap-2 text-slate-600 select-none shrink-0 text-xs">
                <div className="flex items-center gap-1 border-r border-slate-200 pr-2">
                  <button title="Undo (Ctrl+Z)" className="p-1 hover:bg-slate-200 rounded text-slate-400 cursor-not-allowed">↩</button>
                  <button title="Redo (Ctrl+Y)" className="p-1 hover:bg-slate-200 rounded text-slate-400 cursor-not-allowed">↪</button>
                  <button title="Print (Ctrl+P)" className="p-1 hover:bg-slate-200 rounded text-slate-500 hover:text-slate-800"><Printer className="w-3.5 h-3.5" /></button>
                </div>
                ...
                <div className="flex items-center gap-1 border-r border-slate-200 pr-2">
                  <button title="Bold (Ctrl+B)" className="p-1 hover:bg-slate-200 rounded font-bold">B</button>
                  <button title="Italic (Ctrl+I)" className="p-1 hover:bg-slate-200 rounded italic">I</button>
                  <button title="Underline (Ctrl+U)" className="p-1 hover:bg-slate-200 rounded underline">U</button>
                  <button title="Strikethrough" className="p-1 hover:bg-slate-200 rounded line-through">S</button>
                </div>
                ...
                <div className="flex items-center gap-1 border-r border-slate-200 pr-2">
                  ...
                  <span className="font-bold text-indigo-600 cursor-pointer text-xs">A</span>
                </div>
                ...
                <div className="flex-1 flex items-center justify-end gap-2 min-w-[200px]">
                  <div className="relative w-full max-w-xs">
                    ...
                    />
                    {articulationSearch && (
                      <button onClick={() => setArticulationSearch('')} className="absolute right-2 top-1.5 text-slate-400 hover:text-slate-600 font-bold text-[10px]">×</button>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border-b border-slate-200 px-4 py-1 flex items-center text-xs shrink-0 font-mono">
                <div className="bg-slate-50 border border-slate-200 px-2 py-0.5 rounded text-center min-w-[40px] font-bold text-[#107c41]">
                  {activeCellId ? `${activeCellId.col}${activeCellId.row}` : 'A1'}
                </div>
                <div className="h-4 w-[1px] bg-slate-300 mx-2" />
                <div className="text-slate-400 font-serif italic font-semibold select-none mr-2">fx</div>
                <input 
                  type="text" 
                  readOnly
                  value={(() => {
                    if (spreadsheetFilteredVariants.length === 0) return '=EMPTY_SELECTION()';
                    const activeRowIndex = Math.min((activeCellId?.row || 1) - 1, spreadsheetFilteredVariants.length - 1);
                    const activeVar = spreadsheetFilteredVariants[activeRowIndex >= 0 ? activeRowIndex : 0];
                    if (!activeVar) return `=SUM(F1:F0)`;
                    
                    const col = activeCellId?.col || 'A';
                    const rowQty = spreadsheetQuantities[activeVar.id] || 1;
                    switch (col) {
                      case 'A': return `="${activeVar.name.split(' - ')[0]}"`;
                      case 'B': return `="${activeVar.color}"`;
                      case 'C': return `="${activeVar.size}"`;
                      case 'D': return `="${activeVar.sku}"`;
                      case 'E': return `="${activeVar.barcode}"`;
                      case 'F': return `=STOCK_LEVEL("${activeVar.sku}", ${activeVar.stock})`;
                      case 'G': return `=ORDER_QTY("${activeVar.sku}", ${rowQty})`;
                      case 'H': return `=BUY_SINGLE("${activeVar.sku}", 1)`;
                      case 'I': return `=BUY_BULK("${activeVar.sku}", ${rowQty})`;
                      default: return `=SUM(F2:F${spreadsheetFilteredVariants.length + 1})`;
                    }
                  })()}
              ...
              <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                <div className="flex-1 overflow-auto bg-slate-200 p-1 min-h-[350px] flex flex-col justify-between">
                    ...
                    <table className="w-full border-collapse text-left text-[11px] font-mono table-fixed">
                      <thead>
                        ...
                        <tr className="bg-slate-100 text-slate-500 border-b border-slate-300 text-center text-[10px] font-bold select-none">
                          <th className="w-10 bg-slate-100 border-r border-slate-300 p-1 text-slate-400">#</th>
                          <th className="w-48 border-r border-slate-300 p-1">A (Product Details)</th>
                          <th className="w-20 border-r border-slate-300 p-1">B (Color)</th>
                          <th className="w-14 border-r border-slate-300 p-1">C (Size)</th>
                          <th className="w-28 border-r border-slate-300 p-1">D (SKU)</th>
                          <th className="w-28 border-r border-slate-300 p-1">E (Barcode)</th>
                          <th className="w-22 border-r border-slate-300 p-1">F (In Stock)</th>
                          <th className="w-24 border-r border-slate-300 p-1">G (Order Qty)</th>
                          <th className="w-22 border-r border-slate-300 p-1">H (Buy Single)</th>
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {spreadsheetFilteredVariants.length === 0 ? (
                          <tr>
                            <td colSpan={10} className="p-8 text-center text-slate-400 font-sans italic bg-slate-50">
                              No matching variant matrix cells found. Adjust spreadsheet filters.
                            </td>
                          </tr>
                        ) : (
                          spreadsheetFilteredVariants.map((item, index) => {
                            const rowIndex = index + 1;
                            const itemQty = spreadsheetQuantities[item.id] || 1;
                            const isLow = item.stock <= item.minStockAlert;
                            const isOut = item.stock <= 0;

                            const checkCellSelected = (colName: string) => {
                              return activeCellId?.row === rowIndex && activeCellId?.col === colName;
                            };

                            const renderCellBorderClass = (colName: string) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'A' });
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'B' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'C' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'D' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'E' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'F' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'G' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                >
                                  <div className="inline-flex items-center gap-1 bg-white border border-slate-300 rounded px-1 py-0.2">
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const newVal = Math.max(1, itemQty - 1);
                                        setSpreadsheetQuantities(prev => ({ ...prev, [item.id]: newVal }));
                                      }}
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setSpreadsheetQuantities(prev => ({ ...prev, [item.id]: val }));
                                      }}
                                    ...
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const newVal = Math.min(item.stock || 99, itemQty + 1);
                                        setSpreadsheetQuantities(prev => ({ ...prev, [item.id]: newVal }));
                                      }}
                                      className="w-4 h-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded flex items-center justify-center font-bold text-[10px] cursor-pointer"
                                    >
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'H' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                <td 
                                  onClick={() => {
                                    setActiveCellId({ row: rowIndex, col: 'I' });
                                    setSelectedVariant(item);
                                  }}
                                ...
                                >
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleAddProductToCartWithQty(item, itemQty);
                                      onAddNotification(
                                        'Spreadsheet Dispatch', 
                        ...
                        {spreadsheetFilteredVariants.length > 0 && (
                          <tr className="bg-slate-50 font-bold border-t-2 border-slate-300 text-slate-700">
                            ...
                            <td className="p-2 border-r border-slate-200 font-sans text-[10px] uppercase text-slate-500">
                              Spreadsheet Formula Sum (=SUM(F2:F{spreadsheetFilteredVariants.length + 1}))
                            </td>
                            ...
                            <td className="p-2 border-r border-slate-200 text-right text-indigo-600 font-extrabold text-xs underline decoration-double">
                              {spreadsheetFilteredVariants.reduce((sum, v) => sum + v.stock, 0)} Pcs
                            </td>
                            ...
                            <td className="p-2 border-r border-slate-200 text-center text-emerald-600 font-extrabold text-xs">
                              {spreadsheetFilteredVariants.reduce((sum, v) => sum + (spreadsheetQuantities[v.id] || 1), 0)} Pcs
                            </td>
                            ...
                            <td className="p-2 text-indigo-600 text-right font-extrabold text-xs underline decoration-double">
                              ₹{spreadsheetFilteredVariants.reduce((sum, v) => sum + (v.sellingPrice * (spreadsheetQuantities[v.id] || 1)), 0).toLocaleString()}
                            </td>
                          </tr>
                        )}
                      </tbody>
                  ...
                  <div className="mt-1 bg-white border border-slate-300 rounded shadow-xs p-1 px-3 flex items-center justify-between text-[10px] font-sans text-slate-500 shrink-0 select-none">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button className="text-slate-400 hover:text-slate-700 text-sm font-bold ml-1" title="Add New Sheet">+</button>
                    </div>
                    <div className="flex items-center gap-3 font-mono text-[9px]">
                      <span>AVERAGE: {(spreadsheetFilteredVariants.reduce((sum, v) => sum + v.stock, 0) / (spreadsheetFilteredVariants.length || 1)).toFixed(1)}</span>
                      <span>COUNT: {spreadsheetFilteredVariants.length}</span>
                      <span className="font-bold text-slate-700">SUM: {spreadsheetFilteredVariants.reduce((sum, v) => sum + v.stock, 0)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full lg:w-72 bg-slate-900 text-slate-200 p-5 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-slate-800 shrink-0 space-y-5">
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono tracking-widest text-[#107c41] uppercase font-bold">Inspect Workspace Cell</span>
                      <h4 className="text-sm font-extrabold tracking-tight">Active Matrix Blueprint</h4>
                    </div>
                    ...
                    {selectedVariant ? (
                      <div className="space-y-4 text-xs">
                        ...
                        <div className="space-y-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800 font-mono text-[10px] text-slate-300">
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            ...
                            <span className="font-sans font-bold text-white truncate max-w-[150px]">{selectedVariant.brand} {selectedVariant.size}</span>
                          </div>
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            <span className="text-slate-500">Color Variant:</span>
                            <span className="font-sans font-bold text-emerald-400">{selectedVariant.color}</span>
                          </div>
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            <span className="text-slate-500">Size Code:</span>
                            <span className="font-sans font-bold text-white">{selectedVariant.size}</span>
                          </div>
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            <span className="text-slate-500">Item Price:</span>
                            <span className="text-white font-bold font-mono">₹{selectedVariant.sellingPrice}</span>
                          </div>
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            <span className="text-slate-500">Stock Capacity:</span>
                            <span className={selectedVariant.stock <= selectedVariant.minStockAlert ? 'text-amber-400 font-bold' : 'text-emerald-400 font-bold'}>
                              {selectedVariant.stock} units
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-slate-850 pb-1">
                            <span className="text-slate-500">Central Hub:</span>
                            <span className="text-white font-bold">Central Rack {selectedVariant.id.replace('p-', 'C-')}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-500">GST Percent:</span>
                            <span className="text-indigo-400 font-bold">{selectedVariant.gstPercent}%</span>
                          </div>
                        </div>

                        {/* Stock distribution cross warehouses */}
                        <div className="space-y-1.5">
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-sans">Multi-Warehouse Inventory</p>
                          <div className="bg-slate-950/50 border border-slate-800 rounded-lg p-2.5 space-y-1 font-mono text-[9px] text-slate-400">
                            <div className="flex justify-between">
                              <span>Main Warehouse</span>
                              <span className="text-white font-bold">{Math.round(selectedVariant.stock * 0.5)} Qty</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Showroom Rack</span>
                              <span className="text-white font-bold">{Math.round(selectedVariant.stock * 0.3)} Qty</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Stock Transit</span>
                              <span className="text-white font-bold">{Math.round(selectedVariant.stock * 0.2)} Qty</span>
                            </div>
                          </div>
                        </div>
                        ...
                        {activeCustomer && (
                          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 rounded-xl text-[10px] leading-tight font-sans">
                            ...
                            <p className="mt-0.5 font-bold text-white">{activeCustomer.name}</p>
                            <p className="text-[9px] text-slate-400">Membership: <span className="text-emerald-400 font-bold">{activeCustomer.membership}</span></p>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="py-12 text-center text-slate-500 text-xs">
                        <p>No cell active.</p>
                        <p className="text-[10px]">Select any spreadsheet cell to retrieve catalog specs.</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 pt-3 border-t border-slate-800">
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1 text-xs">
                      <div className="flex justify-between text-slate-400 font-mono text-[10px]">
                        ...
                        <span>₹{selectedVariant ? selectedVariant.sellingPrice : 0}</span>
                      </div>
                      <div className="flex justify-between text-slate-400 font-mono text-[10px]">
                        <span>Order Dispatch Qty:</span>
                        <span>{selectedVariant ? (spreadsheetQuantities[selectedVariant.id] || 1) : 0} units</span>
                      </div>
                      <div className="flex justify-between items-center pt-1.5 border-t border-slate-800 text-white font-bold text-xs">
                        ...
                        <span className="font-mono text-emerald-400 text-sm">
                          ₹{selectedVariant ? (selectedVariant.sellingPrice * (spreadsheetQuantities[selectedVariant.id] || 1)).toLocaleString() : 0}
                        </span>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (selectedVariant) {
                          const rowQty = spreadsheetQuantities[selectedVariant.id] || 1;
                          handleAddProductToCartWithQty(selectedVariant, rowQty);
                          onAddNotification(
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-scale-up">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
              <div className="flex items-center gap-1.5 text-emerald-600">
                ...
                <span className="text-sm font-bold uppercase tracking-wide">Sale Completed Successfully</span>
              </div>
            ...
            <div className="border border-slate-100 rounded-xl p-4 bg-slate-50 font-mono text-xs text-slate-800 space-y-3 max-h-96 overflow-y-auto">
              <div className="text-center font-bold text-slate-900 text-sm">ZIVA FASHION BOUTIQUE</div>
              <div className="text-center text-[10px] text-slate-500">Bandra, Mumbai - GSTIN 27AABCV1942A1ZX</div>
              ...
              <div className="border-t border-dashed border-slate-300 my-2" />
              <div className="text-center text-[10px] text-slate-400">Powering Retail Commerce via GarmentFlow SaaS</div>
            </div>
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-3">
                ...
                <button 
                  onClick={() => {
                    onAddNotification('Printer Terminal', 'Sending print job TM-T88 thermal stack...', 'info');
                    onAddNotification('SMS Gateway', `WhatsApp receipt sent to ${completedInvoice.customerPhone}`, 'success');
