...
import { PurchaseOrder, Supplier, Product } from '../types/erp';
...
interface PurchaseViewProps {
  ...
  onSettleSupplierBalance: (supplierId: string, amount: number) => void;
...
export const PurchaseView: React.FC<PurchaseViewProps> = ({
  ...
  const paginatedPOs = purchaseOrders.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  const activeSupplierPayoutObj = suppliers.find(s => s.id === selectedSupplierPayoutId);
  ...
  const handleCreatePOSubmit = (e: React.FormEvent) => {
    ...
    onAddNotification('Supply Chain Ledger', `Compiled Purchase Order ${newPO.poNo} for ${newPO.grandTotal.toLocaleString()} pending dispatch.`, 'success');
    setShowPOModal(false);
    setPoSupplierId('');
    setPoProductId('');
  };

  const handleSettleSupplierPayout = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSupplierPayoutId || payoutAmount <= 0) return;

    if (activeSupplierPayoutObj && activeSupplierPayoutObj.outstandingBalance < payoutAmount) {
      onAddNotification('Payout Warning', 'Payout amount exceeds outstanding balance ledger.', 'warning');
      return;
    }
    ...
    <div className="space-y-6 animate-fade-in pb-12" id="purchase-mgmt-root">
      ...
      <div className="flex border-b border-slate-100 pb-3">
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button onClick={() => setActiveTab('pos')} className={`px-4 py-2 rounded-lg text-xs font-semibold cursor-pointer ${activeTab === 'pos' ? 'bg-white text-slate-800' : 'text-slate-500 hover:text-slate-800'}`}>
            Purchase Orders (PO)
          </button>
          <button onClick={() => setActiveTab('suppliers')} className={`px-4 py-2 rounded-lg text-xs font-semibold cursor-pointer ${activeTab === 'suppliers' ? 'bg-white text-slate-800' : 'text-slate-500 hover:text-slate-800'}`}>
            Wholesale Suppliers
          </button>
        </div>
      </div>
      ...
      {activeTab === 'pos' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            <div>
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Active Procurement Pipelines</h3>
              <p className="text-xs text-slate-400">Total recorded supplier receipts: {purchaseOrders.length} orders</p>
            </div>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {paginatedPOs.map((po, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5 font-mono font-bold text-indigo-600">{po.poNo}</td>
                      <td className="p-3.5">{po.date}</td>
                      <td className="p-3.5 text-slate-800 font-semibold">{po.supplierName}</td>
                      <td className="p-3.5 max-w-[200px] truncate">
                        {po.items.map(item => `${item.quantity}x ${item.name}`).join(', ')}
                      </td>
                      <td className="p-3.5 text-right font-mono">₹{po.subTotal.toLocaleString()}</td>
                      <td className="p-3.5 text-right font-mono text-slate-400">₹{po.gstTotal.toLocaleString()}</td>
                      <td className="p-3.5 text-right font-mono font-bold text-slate-800">₹{po.grandTotal.toLocaleString()}</td>
                      <td className="p-3.5 text-center">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${po.status === 'Completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600 animate-pulse'}`}>
                          {po.status === 'Completed' ? 'Fulfilled' : 'Dispatched'}
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      {activeTab === 'suppliers' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 lg:col-span-4 space-y-4 text-xs">
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Disburse Ledger Payout</h4>
            <form onSubmit={handleSettleSupplierPayout} className="space-y-3">
              ...
              {activeSupplierPayoutObj && (
                <div className="bg-slate-50 p-4 rounded-xl space-y-1 border border-slate-100 font-mono">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-800 font-bold">{activeSupplierPayoutObj.name}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-red-500 font-bold">₹{activeSupplierPayoutObj.outstandingBalance.toLocaleString()}</span>
                  </div>
                </div>
              )}

              <div>
                <label className="block text-slate-400 mb-1 font-semibold">Payout Amount (₹)</label>
                <input 
                  required
                  type="number" 
                  value={payoutAmount || ''} 
              ...
              <button type="submit" className="w-full py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-md">
                Disburse Payout
              </button>
            </form>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden lg:col-span-8">
            <div className="p-4 border-b border-slate-100">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wide">Registered Textile Wholesalers</h4>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                  {suppliers.map((sup, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5">
                        <p className="font-bold text-slate-800 leading-tight">{sup.name}</p>
                        <span className="text-[10px] text-slate-400">{sup.totalOrders} completed purchase loops</span>
                      </td>
                      <td className="p-3.5">{sup.contactPerson}</td>
                      <td className="p-3.5 font-mono text-slate-500 font-bold uppercase">{sup.gstin}</td>
                      <td className="p-3.5">
                        <p>{sup.phone}</p>
                      ...
                      <td className="p-3.5 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${sup.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                          {sup.status}
                        </span>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl border border-slate-100 animate-scale-up">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
              <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Draft Procurement PO</h4>
              <button onClick={() => setShowPOModal(false)} className="text-slate-400 hover:text-slate-600"><Check className="w-5 h-5 rotate-45" /></button>
            </div>

            <form onSubmit={handleCreatePOSubmit} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-slate-500 mb-1 font-semibold">Select Wholesaler *</label>
                <select required value={poSupplierId} onChange={(e) => setPoSupplierId(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-slate-700">
              ...
              <div>
                <label className="block text-slate-500 mb-1 font-semibold">Garment SKU style *</label>
                <select required value={poProductId} onChange={(e) => setPoProductId(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-slate-700">
              ...
              <div>
                <label className="block text-slate-500 mb-1 font-semibold">Procurement Quantity (Units)</label>
                <input 
                  type="number" 
                  min={1} 
                  required 
              ...
              <div className="flex gap-2 justify-end border-t border-slate-100 pt-3">
                <button type="button" onClick={() => setShowPOModal(false)} className="px-4 py-2 bg-slate-100 rounded-xl font-semibold">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-slate-950 text-white rounded-xl font-bold">Submit PO</button>
              </div>
            </form>
          </div>
        </div>
