...
import { CustomGarmentDetails } from '../types/erp';

interface ArticulationViewProps {
  onAddCustomToCart: (customItem: CustomGarmentDetails) => void;
  onAddNotification: (title: string, msg: string, type: 'info' | 'warning' | 'success' | 'danger') => void;
}

export const ArticulationView: React.FC<ArticulationViewProps> = ({
  onAddCustomToCart,
  onAddNotification
}) => {
  ...
  const patterns = ['Solid Plain', 'Classic Stripes', 'Windowpane Checks', 'Floral Motif', 'Self-Weave Textured'];
  const sleeveTypes = ['Full Sleeves', 'Half Sleeves', 'Sleeveless', 'Three-Quarter'];
  const neckTypes = ['Standard Collar', 'Mandarin Collar', 'Crew Neck', 'V-Neck', 'Double Button Band'];
  ...
  const handlePushToPOS = () => {
    const customGarment: CustomGarmentDetails = {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12" id="articulation-builder-root">
      {/* Module Title */}
      <div>
        <h1 className="text-xl font-bold text-slate-800 uppercase tracking-wider">Garment Articulation & Bespoke Studio</h1>
        <p className="text-xs text-slate-400">Configure fabric weaves, custom tailoring dimensions, and structural embellishments.</p>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 lg:col-span-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1.5">Fabric Composition *</label>
                <select 
                ...
                >
                  {fabrics.map((fab, idx) => (
                    <option key={idx} value={fab}>{fab} (Base ₹{fabricPrices[fab]})</option>
                  ))}
                </select>
              </div>

              <div>
                <label className="block text-slate-500 font-semibold mb-1.5">Fabric Pattern Structure</label>
                <select 
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-500">Dye Batch & Color Shade</label>
              <div className="flex flex-wrap gap-2.5">
                {colors.map((c, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-4.5 h-4.5 rounded-md border border-slate-200" style={{ backgroundColor: c.hex }} />
                      <span>{c.name}</span>
                      {isSelected && <Check className="w-3.5 h-3.5 ml-1 text-blue-600" />}
                    </button>
                  );
                })}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
              <div>
                <label className="block text-slate-500 font-semibold mb-1.5">Sleeve Cut Type</label>
                <select 
              ...
              <div>
                <label className="block text-slate-500 font-semibold mb-1.5">Collar/Neck Style</label>
                <select 
                  value={selectedNeck}
                ...
                >
                  {neckTypes.map((nk, idx) => (
                    <option key={idx} value={nk}>{nk}</option>
                  ))}
                </select>
              </div>

              <div>
                <label className="block text-slate-500 font-semibold mb-1.5">Base Sizing Blueprint</label>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
              <div className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-100 rounded-xl">
                <div className="space-y-0.5">
                  <span className="font-semibold text-slate-700 block">Zari Embroidery</span>
                  <span className="text-[10px] text-slate-400 block font-mono">+₹450 Charge</span>
                </div>
                <input 
                  type="checkbox" 
                  checked={embroidery} 
              ...
              <div className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-100 rounded-xl">
                <div className="space-y-0.5">
                  <span className="font-semibold text-slate-700 block">Logo / Silk Screen</span>
                  <span className="text-[10px] text-slate-400 block font-mono">+₹250 Charge</span>
                </div>
                <input 
                  type="checkbox" 
              ...
              <div>
                <label className="block text-slate-500 font-semibold mb-1">Tailor Alteration Surcharge</label>
                <div className="relative">
                  <span className="absolute left-3 top-2 text-slate-400 font-semibold font-mono">₹</span>
                  <input 
                    type="number" 
                    value={alterationCharges || ''} 
                    onChange={(e) => setAlterationCharges(Math.max(0, Number(e.target.value)))}
            ...
            <div className="space-y-1.5 text-xs">
              <label className="block text-slate-500 font-semibold">Special Instructions for Tailoring Line</label>
              <textarea 
        ...
        <div className="space-y-5 lg:col-span-4">
          <div className="bg-slate-900 rounded-2xl shadow-xl text-white p-6 space-y-6 border border-slate-800 relative overflow-hidden">
            ...
            <div className="space-y-1">
              <span className="text-[9px] font-mono tracking-widest text-indigo-400 uppercase">Live Articulated Spec</span>
              <h3 className="text-base font-bold tracking-tight">Interactive Visual Blueprint</h3>
            </div>
            ...
            <div className="h-44 border border-slate-800 rounded-xl bg-slate-950/80 flex flex-col items-center justify-center p-4 relative font-mono text-[11px] text-slate-400">
              ...
              <div className="flex flex-col items-center space-y-1">
                ...
                <span className="text-white font-bold">{selectedFabric} Jacket / Shirt</span>
                <span className="text-slate-500 uppercase">{selectedPattern} - {selectedColor.name}</span>
            ...
            <div className="space-y-2.5 text-xs text-slate-300 font-mono">
              ...
              <div className="flex justify-between border-b border-slate-800 pb-1">
                ...
                <span className={embroidery ? 'text-emerald-400' : 'text-slate-600'}>{embroidery ? 'Yes' : 'No'}</span>
              </div>
              <div className="flex justify-between border-b border-slate-800 pb-1">
                <span className="text-slate-500">Screen logo</span>
                <span className={logoPrinting ? 'text-emerald-400' : 'text-slate-600'}>{logoPrinting ? 'Yes' : 'No'}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono text-slate-300">₹{estimatedCost}</span>
              </div>
              <div className="flex justify-between items-center text-sm font-bold border-t border-slate-800 pt-2 text-white">
                ...
                <span className="font-mono text-indigo-400 text-lg">₹{estimatedCost}</span>
              </div>
            </div>
