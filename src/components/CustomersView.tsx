...
interface CustomersViewProps {
  customers: Customer[];
  onSettleCustomerBalance: (customerId: string, amount: number) => void;
...
export const CustomersView: React.FC<CustomersViewProps> = ({
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    const matchesMembership = membershipFilter === 'All' || c.membership === membershipFilter;
    return matchesSearch && matchesMembership;
  });

  const handleSettleCustomerSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCustomerId || settleAmount <= 0) return;

    if (activeCustomer.outstandingBalance < settleAmount) {
      onAddNotification('CRM Ledger Alert', 'Settlement exceeds total outstanding customer debt.', 'warning');
      return;
    }
  ...
  const handleTriggerBirthdayPromo = (cust: Customer) => {
    onAddNotification(
    ...
    <div className="space-y-6 animate-fade-in pb-12" id="customers-crm-root">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 lg:col-span-4 space-y-4 text-xs">
          <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Settle Credit Outstanding</h4>
          <form onSubmit={handleSettleCustomerSubmit} className="space-y-4">
            <div>
              <label className="block text-slate-400 mb-1 font-semibold">Select Customer Debt Account</label>
              <select 
              ...
              >
                {customers.map(c => (
                  <option key={c.id} value={c.id}>{c.name} (Outstanding: ₹{c.outstandingBalance})</option>
                ))}
              </select>
            </div>
            ...
            {activeCustomer && (
              <div className="bg-slate-50 p-4 rounded-xl space-y-1.5 border border-slate-100 font-mono">
                <div className="flex justify-between">
                  ...
                  <span className="text-indigo-600 font-bold uppercase">{activeCustomer.membership}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-red-500 font-bold">₹{activeCustomer.outstandingBalance.toLocaleString()}</span>
                </div>
              </div>
            )}

            <div>
              <label className="block text-slate-400 mb-1 font-semibold">Payment Amount Settle (₹)</label>
              <input 
                type="number" 
                value={settleAmount || ''} 
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden lg:col-span-8">
          <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-semibold">
            ...
            <div className="flex gap-2">
              ...
              />
              <select value={membershipFilter} onChange={(e) => setMembershipFilter(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                {filteredCustomers.slice(0, 10).map((cust, idx) => {
                  ...
                  const tierColor = cust.membership === 'Platinum' ? 'bg-slate-950 text-amber-400' : (cust.membership === 'Gold' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700');
                  return (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="p-3.5">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <span className={`text-[9px] px-1.5 py-0.2 rounded font-mono font-bold ${tierColor}`}>{cust.membership}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-3.5">
                      ...
                      <td className="p-3.5 text-center font-bold font-mono text-violet-600">{cust.loyaltyPoints} LP</td>
                      <td className={`p-3.5 text-right font-mono font-bold ${cust.outstandingBalance > 0 ? 'text-red-500' : 'text-slate-400'}`}>
                      ...
                      <td className="p-3.5 text-center">
                        {isBdayToday ? (
                          ...
                          <span className="text-slate-400 font-mono text-[10px]">{cust.birthday}</span>
                        )}
                      </td>
                    </tr>
                  );
