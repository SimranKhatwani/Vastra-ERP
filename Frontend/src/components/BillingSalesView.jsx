...
const normalizeInvoice = (b) => {
  ...
  return {
    ...
    items: rawItems.map(i => ({
      ...
      designNo: i.designNo || i.sku || i.itemCode || i.inventoryPieceId?.productId?.designNo || i.productId?.designNo || '',
      sku: i.sku || i.designNo || '',
      itemCode: i.itemCode || '',
      barcode: i.barcode || i.barcodeNo || i.inventoryPieceId?.barcode || '',
      uniqueCode: i.uniqueCode || i.inventoryPieceId?.uniqueCode || i.barcode || '',
            ...
            <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto text-xs">
              ...
              <div className="bg-slate-900 text-white rounded-2xl p-4 text-center space-y-2 font-mono shadow-md border border-slate-800">
                <div className="flex items-center justify-center gap-1.5">
                  ...
                  <p className="text-[10.5px] text-emerald-300 font-black uppercase tracking-wider">ALTERATION TRACKING QR CODE ({pssSlipModalData.pssmNo})</p>
                </div>
                <div className="bg-white p-3 rounded-xl inline-block text-slate-900 shadow-inner max-w-full">
                  {(() => {
                    const pssmQrSvg = generateInvoiceUPIQrSvg(pssSlipModalData, {
                      width: 130,
                      height: 130,
                      padding: 1
                    });
                ...
                <p className="text-[9.5px] text-slate-300">
                  Scan to View Live Alteration Progress • Linked to Ticket <strong className="text-white font-mono">{pssSlipModalData.pssmNo}</strong>
                </p>
              </div>
            </div>
