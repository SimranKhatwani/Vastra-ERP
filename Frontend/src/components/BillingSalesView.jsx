      ...
      if (res.data?.success && res.data.data?.pssm) {
        ...
        setPssSlipModalData({
          ...
          items: items.map((it, idx) => ({
            ...
            gender: it.gender || 'Gents',
            measurements: it.measurements || {},
            charge: it.charge || 0,
            trialRequired: it.trialRequired,
            trialDate: it.trialDate,
            ...
            <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto text-xs">
                ...
                {(pssSlipModalData.items || []).map((it, i) => {
                  ...
                  return (
                    <div key={i} className={`border rounded-xl p-2.5 space-y-1.5 transition-all ${
                      ...
                      {(() => {
                        const itemAltBarcode = it.alterationBarcode || it.tailorInvoiceNo || (it.barcode && String(it.barcode).startsWith('TI-') ? it.barcode : null) || (pssSlipModalData.pssmNo ? `${pssSlipModalData.pssmNo}-${i + 1}` : null);
              ...
              <div className="bg-slate-900 text-white rounded-2xl p-4 text-center space-y-2 font-mono shadow-md border border-slate-800">
                <div className="flex items-center justify-center gap-1.5">
                  ...
                  <p className="text-[10.5px] text-emerald-300 font-black uppercase tracking-wider">CONNECTED INVOICE UPI &amp; VERIFICATION QR CODE</p>
                </div>
                <div className="bg-white p-3 rounded-xl inline-block text-slate-900 shadow-inner max-w-full">
                  {(() => {
                    const billQrSvg = generateInvoiceUPIQrSvg(pssSlipModalData, {
                      width: 130,
                      height: 130,
                      padding: 1
                    });
                    return (
                      <div className="flex flex-col items-center justify-center">
                        <div
                          ...
                          dangerouslySetInnerHTML={{ __html: billQrSvg }}
                        />
                        <div className="text-[11px] font-black mt-1 font-mono tracking-wider text-slate-900">
                          {pssSlipModalData.originalInvoiceNo || pssSlipModalData.billBarcode || pssSlipModalData.pssmNo}
                        </div>
                      </div>
                    );
                  })()}
                ...
                <p className="text-[9.5px] text-slate-300">
                  Scan &amp; Pay with any UPI App • Same QR code as connected invoice <strong className="text-white font-mono">{pssSlipModalData.originalInvoiceNo}</strong>
                </p>
              </div>
            </div>
