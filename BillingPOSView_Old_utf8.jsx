...
export const BillingPOSView = ({
    ...
    return filteredProducts.map((p, idx) => {
      ...
      return (
        ...
        >
          ...
          <td className="p-2.5 text-[11px] text-slate-400 font-mono line-through">Γé╣{p.mrp || p.price}</td>
          <td className="p-2.5 text-xs font-bold text-indigo-600 font-mono">
            Γé╣{p.sellingPrice || p.price}
          </td>
  ...
  const handleCheckoutSubmit = async () => {
    ...
    if (paymentMethod === "Credit") {
      ...
      onAddNotification(
        ...
        `Γé╣${grandTotal.toLocaleString()} logged to ${activeCustomer.name}'s credit ledger.`,
        "info",
      );
    }
    ...
    // Trigger state callbacks ΓÇö onAddInvoice now returns the saved invoice object (or null on error)
    ...
    onAddNotification(
      ...
      `Issued receipt ${newInvoice.invoiceNo} for Γé╣${newInvoice.grandTotal.toLocaleString()}`,
      "success",
    );

    // ΓöÇΓöÇ Automatic WhatsApp Dispatch (fire-and-forget, never blocks checkout) ΓöÇΓöÇ
  ...
  const handleCreateQuotation = (e) => {
    ...
    onAddNotification(
      "Quotation Compiled",
      `Quote estimate generated for ${quoteCustName}. Value: Γé╣${total.toLocaleString()}`,
      "success",
    );
  ...
  const handleCreateCreditNote = (e) => {
    ...
    onAddNotification(
      "Credit Note Issued",
      `Credited Γé╣${creditAmt.toLocaleString()} to ${targetC.name}'s wallet ledger.`,
      "success",
    );
  ...
  const handleCreateDebitNote = (e) => {
    ...
    onAddNotification(
      "Debit Note Levied",
      `Charged Γé╣${debitAmt.toLocaleString()} to ${targetC.name}'s balance account. Reason: ${debitReason}`,
      "success",
    );
  ...
  const handleSubmitReturn = () => {
    ...
    onAddNotification(
      "Return Approved",
      `Returned items from ${selectedInvoiceForReturn.invoiceNo}. Credited Γé╣${refundTotal.toLocaleString()} to customer's account balance.`,
      "success",
    );
    // Clear states
              ...
              ${item.hasAlteration || item.alterationRecord ? `
                <tr>
                  <td colSpan="4" style="font-size:9.5px; color:#be123c; background:#fff1f2; padding:4px 6px; border-radius:4px; margin-bottom:4px;">
                    <b>Γ£é ALTERATION:</b> ${item.alterationRecord?.alterationDetails?.join(', ') || 'Custom Fit'} | <b>Tailor:</b> ${item.alterationRecord?.tailorName || 'Master Tailor'}<br/>
  ...
  const handleWhatsAppShare = (invoice) => {
    const itemsText = invoice.items
      .map(
        (item) =>
          `- ${item.name} (${item.size}/${item.color}) x ${item.quantity} = Γé╣${item.totalPrice}`,
      )
      .join("\n");
    const msg =
      ...
      `*Apparel Items:*\n${itemsText}\n` +
      `---------------------------\n` +
      `*Subtotal:* Γé╣${invoice.subTotal}\n` +
      `*GST (CGST+SGST):* Γé╣${invoice.gstTotal}\n` +
      `*Grand Total:* Γé╣${invoice.grandTotal}\n\n` +
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 lg:col-span-5 flex flex-col h-[calc(100vh-140px)] gap-4 relative">
              ...
              <div className="flex justify-between items-center bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-[11px] font-mono">
                <div>
                  ...
                  <span className="text-emerald-600 font-bold">
                    Γé╣{activeCustomer.walletBalance || 0}
                  </span>
                </div>
                <div>
                ...
                {activeCustomer.outstandingBalance > 0 && (
                  <div>
                    ...
                    <span className="text-red-500 font-bold">
                      Γé╣{activeCustomer.outstandingBalance}
                    </span>
                  </div>
                )}
              </div>
                    ...
                    <div key={idx} className="bg-white border border-slate-100 rounded-lg p-2.5 flex flex-col space-y-1.5 shadow-sm">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className="text-xs font-bold text-indigo-600 shrink-0 font-mono">
                          Γé╣{(Number(item.totalPrice) || 0).toLocaleString()}
                        </span>
                      </div>
            ...
            <div className="shrink-0 space-y-3 pt-2">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <div className="relative">
                    <span className="text-slate-400 absolute left-2 top-1.5 text-xs font-mono font-bold">Γé╣</span>
                    <input
                      type="number"
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs space-y-2">
                <div className="flex justify-between text-slate-500">
                  ...
                  <span className="font-mono font-semibold">Γé╣{subTotal.toLocaleString()}</span>
                </div>
                {autoOffer && (
                      ...
                      <tbody>
                        {filteredProducts.slice(0, 20).map((p, idx) => (
                          ...
                          >
                            ...
                            <td className="p-2 text-xs font-bold text-indigo-600 font-mono">Γé╣{p.sellingPrice}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                            ...
                            {inv.items.map((item, idx) => (
                              ...
                              >
                                ...
                                <span className="font-bold font-mono text-slate-700">
                                  Γé╣{item.totalPrice.toLocaleString()}
                                </span>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="border-t border-slate-100/60 pt-2 flex justify-between items-center">
                            ...
                            <div className="text-right">
                              ...
                              <span className="font-extrabold text-xs text-slate-800 font-mono">
                                Γé╣{inv.grandTotal.toLocaleString()}
                              </span>
                            </div>
                          </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2.5 text-xs">
                  ...
                  <div className="flex justify-between font-semibold">
                    ...
                    <span className="font-mono text-indigo-600">
                      Γé╣{selectedInvoiceForReturn.grandTotal.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between font-semibold">
                          ...
                          <div className="space-y-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800 font-mono text-[10px] text-slate-300">
                            ...
                            <div className="flex justify-between border-b border-slate-850 pb-1">
                              ...
                              <span className="text-white font-bold font-mono">
                                Γé╣{selectedVariant.sellingPrice}
                              </span>
                            </div>
                            <div className="flex justify-between border-b border-slate-850 pb-1">
                              <span className="text-slate-500">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-scale-up">
            ...
            <div className="border border-slate-100 rounded-xl p-4 bg-slate-50 font-mono text-xs text-slate-800 space-y-3 max-h-96 overflow-y-auto">
                ...
                {completedInvoice.items.map((item, idx) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between">
                      ...
                      <span>Γé╣{(Number(item.totalPrice) || 0).toLocaleString()}</span>
                    </div>
                    {(item.hasAlteration || item.alterationRecord) && (
              ...
              <div className="space-y-1 text-right">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                  <span>Γé╣{completedInvoice.subTotal}</span>
                </div>
                {completedInvoice.discountTotal > 0 && (
                  <div className="flex justify-between text-emerald-600 font-bold">
                    <span>Discount:</span>
                    <span>-Γé╣{completedInvoice.discountTotal}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span>GST:</span>
                  <span>Γé╣{completedInvoice.gstTotal}</span>
                </div>
                <div className="flex justify-between font-bold text-slate-900 text-sm">
                  <span>Total Paid:</span>
                  <span>Γé╣{completedInvoice.grandTotal}</span>
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              {/* ΓöÇΓöÇ WhatsApp Dispatch Status ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
              ...
              <div className="mb-5 pb-4 border-b border-slate-100 flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs font-medium text-slate-500">
                    ...
                    <span className="text-indigo-600 font-bold">Γé╣{qtyModalProduct.sellingPrice || qtyModalProduct.price}</span>
                ...
                <div>
                  ...
                  >
                    ...
                    {uniqueSizes.map(s => {
                       ...
                       let emoji = "";
                       if (variant) {
                          if (variant.stock <= 0) emoji = "≡ƒö┤ ";
                          else if (variant.stock <= (variant.minStockAlert || 5)) emoji = "≡ƒƒí ";
                          else emoji = "≡ƒƒó ";
                       }
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                      ...
                      <span className="text-[10px] text-slate-400 font-medium">Non-mandatory ΓÇö fill required specs only</span>
                    </div>
